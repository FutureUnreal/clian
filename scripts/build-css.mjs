...
/**
 * CSS Build Script
 * Concatenates modular CSS files from src/style/ into dist/styles.css
 */
...
function build() {
  ...
  try { unlinkSync(join(ROOT, 'styles.css')); } catch { /* ignore */ }
  try { unlinkSync(join(ROOT, 'styles.css.map')); } catch { /* ignore */ }

  mkdirSync(join(ROOT, 'dist'), { recursive: true });
  const moduleOrder = getModuleOrder();
  const parts = ['/* Clian Plugin Styles */\n/* Built from src/style/ modules */\n'];
  const missingFiles = [];
  ...
  console.log(`Built dist/styles.css (${(output.length / 1024).toFixed(1)} KB)`);
}
