...
export class MentionDropdownController {
  ...
  constructor(
    ...
    this.dropdown = new SelectableDropdown<MentionItem>(this.containerEl, {
      ...
      fixed: this.fixed,
      fixedClassName: 'clian-mention-dropdown-fixed',
    });
  }
  ...
  private renderMentionDropdown(): void {
    this.dropdown.render({
      ...
      renderItem: (item, itemEl) => {
        ...
        const textEl = itemEl.createSpan({ cls: 'clian-mention-text' });

        if (item.type === 'mcp-server') {
          const nameEl = textEl.createSpan({ cls: 'clian-mention-name' });
          nameEl.setText(`@${item.name}`);
        } else if (item.type === 'agent-folder') {
          const nameEl = textEl.createSpan({
            cls: 'clian-mention-name clian-mention-name-agent-folder',
          });
          nameEl.setText(`@${item.name}/`);
        } else if (item.type === 'agent') {
          const nameEl = textEl.createSpan({ cls: 'clian-mention-name clian-mention-name-agent' });
          // Show ID (which is namespaced for plugin agents) for consistency with inserted text
          nameEl.setText(`@${item.id}`);
          if (item.description) {
            const descEl = textEl.createSpan({ cls: 'clian-mention-agent-desc' });
            descEl.setText(item.description);
          }
        } else if (item.type === 'context-folder') {
          const nameEl = textEl.createSpan({
            cls: 'clian-mention-name clian-mention-name-folder',
          });
          nameEl.setText(`@${item.name}/`);
        } else if (item.type === 'context-file') {
          const nameEl = textEl.createSpan({
            cls: 'clian-mention-name clian-mention-name-context',
          });
          nameEl.setText(item.name);
        } else if (item.type === 'folder') {
          const nameEl = textEl.createSpan({
          ...
          const pathEl = textEl.createSpan({ cls: 'clian-mention-path' });
