...
class ForkTargetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('clian-fork-target-modal');

    const list = this.contentEl.createDiv({ cls: 'clian-fork-target-list' });
  ...
  private createOption(container: HTMLElement, target: ForkTarget, label: string): void {
    const item = container.createDiv({ cls: 'clian-fork-target-option', text: label });
