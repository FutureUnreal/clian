...
export class InstructionModal extends Modal {
  ...
  onOpen() {
    ...
    contentEl.addClass('clian-instruction-modal');
    this.setTitle('Add Custom Instruction');

    // User input section (always visible)
    const inputSection = contentEl.createDiv({ cls: 'clian-instruction-section' });
    const inputLabel = inputSection.createDiv({ cls: 'clian-instruction-label' });
    inputLabel.setText('Your input:');
    const inputText = inputSection.createDiv({ cls: 'clian-instruction-original' });
    inputText.setText(this.rawInstruction);

    // Main content section (changes based on state)
    this.contentSectionEl = contentEl.createDiv({ cls: 'clian-instruction-content-section' });

    // Loading state
    this.loadingEl = this.contentSectionEl.createDiv({ cls: 'clian-instruction-loading' });
    this.loadingEl.createDiv({ cls: 'clian-instruction-spinner' });
    this.loadingEl.createSpan({ text: 'Processing your instruction...' });

    // Clarification state (hidden initially)
    this.clarificationEl = this.contentSectionEl.createDiv({ cls: 'clian-instruction-clarification-section' });
    this.clarificationEl.style.display = 'none';
    this.clarificationTextEl = this.clarificationEl.createDiv({ cls: 'clian-instruction-clarification' });

    const responseSection = this.clarificationEl.createDiv({ cls: 'clian-instruction-section' });
    const responseLabel = responseSection.createDiv({ cls: 'clian-instruction-label' });
    responseLabel.setText('Your response:');

    this.responseTextarea = new TextAreaComponent(responseSection);
    this.responseTextarea.inputEl.addClass('clian-instruction-response-textarea');
    ...
    this.confirmationEl = this.contentSectionEl.createDiv({ cls: 'clian-instruction-confirmation-section' });
    this.confirmationEl.style.display = 'none';

    // Refined instruction display/edit
    const refinedSection = this.confirmationEl.createDiv({ cls: 'clian-instruction-section' });
    const refinedLabel = refinedSection.createDiv({ cls: 'clian-instruction-label' });
    refinedLabel.setText('Refined snippet:');

    this.refinedDisplayEl = refinedSection.createDiv({ cls: 'clian-instruction-refined' });
    this.editContainerEl = refinedSection.createDiv({ cls: 'clian-instruction-edit-container' });
    this.editContainerEl.style.display = 'none';

    this.editTextarea = new TextAreaComponent(this.editContainerEl);
    this.editTextarea.inputEl.addClass('clian-instruction-edit-textarea');
    this.editTextarea.inputEl.rows = 4;

    // Buttons (changes based on state)
    this.buttonsEl = contentEl.createDiv({ cls: 'clian-instruction-buttons' });
    this.updateButtons();
  ...
  showClarificationLoading() {
    ...
    if (this.loadingEl) {
      this.loadingEl.querySelector('.clian-instruction-spinner');
  ...
  private updateButtons() {
    ...
    const cancelBtn = this.buttonsEl.createEl('button', {
      text: 'Cancel',
      cls: 'clian-instruction-btn clian-instruction-reject-btn',
    ...
    if (this.state === 'clarification') {
      const submitBtn = this.buttonsEl.createEl('button', {
        text: 'Submit',
        cls: 'clian-instruction-btn clian-instruction-accept-btn',
      ...
      this.editBtnEl = this.buttonsEl.createEl('button', {
        text: 'Edit',
        cls: 'clian-instruction-btn clian-instruction-edit-btn',
