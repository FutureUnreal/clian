...
export class ResumeSessionDropdown {
  ...
  constructor(
    ...
    this.dropdownEl = this.containerEl.createDiv({ cls: 'clian-resume-dropdown' });
    this.render();
  ...
  private updateSelection(): void {
    const items = this.dropdownEl.querySelectorAll('.clian-resume-item');
  ...
  private render(): void {
    ...
    const header = this.dropdownEl.createDiv({ cls: 'clian-resume-header' });
    header.createSpan({ text: 'Resume conversation' });

    if (this.conversations.length === 0) {
      this.dropdownEl.createDiv({ cls: 'clian-resume-empty', text: 'No conversations' });
      return;
    }

    const list = this.dropdownEl.createDiv({ cls: 'clian-resume-list' });

    for (let i = 0; i < this.conversations.length; i++) {
      const conv = this.conversations[i];
      const isCurrent = conv.id === this.currentConversationId;

      const item = list.createDiv({ cls: 'clian-resume-item' });
      if (isCurrent) item.addClass('current');
      if (i === this.selectedIndex) item.addClass('selected');

      const iconEl = item.createDiv({ cls: 'clian-resume-item-icon' });
      setIcon(iconEl, isCurrent ? 'message-square-dot' : 'message-square');

      const content = item.createDiv({ cls: 'clian-resume-item-content' });
      const titleEl = content.createDiv({ cls: 'clian-resume-item-title', text: conv.title });
      titleEl.setAttribute('title', conv.title);
      content.createDiv({
        cls: 'clian-resume-item-date',
