 ...
 * These are either handled differently in Clian or don't apply.
 */
...
export class SlashCommandDropdown {
  ...
  private buildCommandList(builtInCommands: SlashCommand[]): SlashCommand[] {
    ...
    // Add Clian built-in commands first (highest priority)
  ...
  private render(): void {
    ...
    if (this.filteredCommands.length === 0) {
      const emptyEl = this.dropdownEl.createDiv({ cls: 'clian-slash-empty' });
      emptyEl.setText('No matching commands');
    } else {
      for (let i = 0; i < this.filteredCommands.length; i++) {
        const cmd = this.filteredCommands[i];
        const itemEl = this.dropdownEl.createDiv({ cls: 'clian-slash-item' });
        ...
        if (cmd.argumentHint) {
          const hintEl = itemEl.createSpan({ cls: 'clian-slash-hint' });
          hintEl.setText(normalizeArgumentHint(cmd.argumentHint));
        }

        if (cmd.description) {
          const descEl = itemEl.createDiv({ cls: 'clian-slash-desc' });
  ...
  private createDropdownElement(): HTMLElement {
    if (this.isFixed) {
      ...
      const dropdown = this.containerEl.createDiv({
        cls: 'clian-slash-dropdown clian-slash-dropdown-fixed',
      });
      return dropdown;
    } else {
      // For chat panel: append to container with absolute positioning
      return this.containerEl.createDiv({ cls: 'clian-slash-dropdown' });
    }
  }
  ...
  private updateSelection(): void {
    const items = this.dropdownEl?.querySelectorAll('.clian-slash-item');
