...
import type { ChatService } from '../../../core/agent';
import type { ChatFlavor, Conversation } from '../../../core/types';
import { t } from '../../../i18n';
...
export interface ConversationControllerDeps {
  ...
  getAgentService?: () => ChatService | null;
}
...
export class ConversationController {
  ...
  private getAgentService(): ChatService | null {
    return this.deps.getAgentService?.() ?? null;
  }

  private getTabFlavor(): ChatFlavor {
    return this.deps.getTabFlavor?.() ?? this.getAgentService()?.flavor ?? 'claude';
  }
  ...
  async createNew(options: { force?: boolean } = {}): Promise<void> {
    ...
    try {
      ...
      welcomeEl.createDiv({ cls: 'clian-welcome-greeting', text: this.getGreeting() });
  ...
  async switchTo(id: string): Promise<void> {
    ...
    try {
      ...
      const tabFlavor = this.getTabFlavor();
      const convFlavor = conversation.flavor ?? 'claude';
      if (convFlavor !== tabFlavor) {
        new Notice(`This tab is ${tabFlavor}; cannot open a ${convFlavor} conversation here.`);
        return;
      }
  ...
  async rewind(userMessageId: string): Promise<void> {
    ...
    if (agentService.flavor !== 'claude' || !('rewind' in agentService)) {
      new Notice('Rewind is only available for Claude sessions.');
      return;
    }

    let result;
    try {
      result = await (agentService as unknown as { rewind: (u: string, a: string) => Promise<any> })
        .rewind(userMsg.sdkUserUuid, prevAssistantUuid);
    } catch (e) {
  ...
  async save(updateLastResponse = false, options?: SaveOptions): Promise<void> {
    ...
    // Non-Claude engines: always persist full JSONL under .clian/sessions
    if (tabFlavor !== 'claude') {
      // Entry point with messages - create conversation lazily
      if (!state.currentConversationId && state.messages.length > 0) {
        const conversation = await plugin.createConversation(sessionId ?? undefined, tabFlavor);
        state.currentConversationId = conversation.id;
      }
    ...
    const sessionInvalidated =
      !!agentService && 'consumeSessionInvalidation' in agentService
        ? (agentService as unknown as { consumeSessionInvalidation: () => boolean }).consumeSessionInvalidation()
        : false;
  ...
  private renderHistoryItems(
    ...
    const dropdownHeader = container.createDiv({ cls: 'clian-history-header' });
    const tabFlavor = this.getTabFlavor();
    dropdownHeader.createSpan({ text: tabFlavor === 'claude' ? 'Conversations' : `Conversations (${tabFlavor})` });

    const list = container.createDiv({ cls: 'clian-history-list' });
    const allConversations = plugin.getConversationList()
      .filter((c) => (c.flavor ?? 'claude') === tabFlavor);

    if (allConversations.length === 0) {
      list.createDiv({ cls: 'clian-history-empty', text: 'No conversations' });
      return;
    }
    ...
    for (const conv of conversations) {
      ...
      const item = list.createDiv({
        cls: `clian-history-item${isCurrent ? ' active' : ''}`,
      });

      const iconEl = item.createDiv({ cls: 'clian-history-item-icon' });
      setIcon(iconEl, isCurrent ? 'message-square-dot' : 'message-square');

      const content = item.createDiv({ cls: 'clian-history-item-content' });
      const titleEl = content.createDiv({ cls: 'clian-history-item-title', text: conv.title });
      titleEl.setAttribute('title', conv.title);
      content.createDiv({
        cls: 'clian-history-item-date',
      ...
      const actions = item.createDiv({ cls: 'clian-history-item-actions' });

      // Show regenerate button if title generation failed, or loading indicator if pending
      if (conv.titleGenerationStatus === 'pending') {
        const loadingEl = actions.createEl('span', { cls: 'clian-action-btn clian-action-loading' });
        setIcon(loadingEl, 'loader-2');
        loadingEl.setAttribute('aria-label', 'Generating title...');
      } else if (conv.titleGenerationStatus === 'failed') {
        const regenerateBtn = actions.createEl('button', { cls: 'clian-action-btn' });
      ...
      const renameBtn = actions.createEl('button', { cls: 'clian-action-btn' });
      ...
      const deleteBtn = actions.createEl('button', { cls: 'clian-action-btn clian-delete-btn' });
  ...
  private showRenameInput(item: HTMLElement, convId: string, currentTitle: string): void {
    const titleEl = item.querySelector('.clian-history-item-title') as HTMLElement;
    if (!titleEl) return;

    const input = document.createElement('input');
    input.type = 'text';
    input.className = 'clian-rename-input';
  ...
  getGreeting(): string {
    ...
    const getTimeGreetings = (): string[] => {
      if (hour >= 5 && hour < 12) {
        return [personalize('Good morning'), 'Coffee and Clian time?'];
  ...
  initializeWelcome(): void {
    ...
    if (!welcomeEl.querySelector('.clian-welcome-greeting')) {
      welcomeEl.createDiv({ cls: 'clian-welcome-greeting', text: this.getGreeting() });
    }
  ...
  async regenerateTitle(conversationId: string): Promise<void> {
    ...
    await titleService.generateTitle(
      ...
      async (convId, result) => {
        ...
        } else if (result.success && !userManuallyRenamed) {
  ...
  // ============================================
  // History Dropdown Rendering (for ClianView)
  // ============================================

  /**
   * Renders the history dropdown content to a provided container.
   * Used by ClianView to render the dropdown with custom selection callback.
   */
