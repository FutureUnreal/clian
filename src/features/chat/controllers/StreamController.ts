...
export class StreamController {
  ...
  private seenAgentOutputToolIds = new Set<string>();
  ...
  private handleRegularToolUse(
    ...
    if (existingToolCall) {
      ...
      if (Object.keys(newInput).length > 0) {
        ...
        if (toolEl) {
          const nameEl = toolEl.querySelector('.clian-tool-name') as HTMLElement | null
            ?? toolEl.querySelector('.clian-write-edit-name') as HTMLElement | null;
          if (nameEl) {
            nameEl.setText(getToolName(existingToolCall.name, existingToolCall.input));
          }
          const summaryEl = toolEl.querySelector('.clian-tool-summary') as HTMLElement | null
            ?? toolEl.querySelector('.clian-write-edit-summary') as HTMLElement | null;
          if (summaryEl) {
  ...
  private async handleToolResult(
    ...
    // Fallback: some Claude Code / SDK versions may emit TaskOutput for the main agent
    // (or omit linking info). If we saw the TaskOutput tool_use but didn't link it to a
    // subagent, treat its result as assistant text rather than dropping it.
    if (this.seenAgentOutputToolIds.has(chunk.id)) {
      this.seenAgentOutputToolIds.delete(chunk.id);
      const text = chunk.content || '';
      if (text.trim()) {
        const prefix = msg.content && !msg.content.endsWith('\n') ? '\n\n' : '';
        msg.content += `${prefix}${text}`;
        await this.appendText(`${prefix}${text}`);
      }
      this.showThinkingIndicator();
      return;
    }
  ...
  async appendText(text: string): Promise<void> {
    ...
    if (!state.currentTextEl) {
      state.currentTextEl = state.currentContentEl.createDiv({ cls: 'clian-text-block' });
  ...
  private handleAgentOutputToolUse(
    ...
    this.seenAgentOutputToolIds.add(chunk.id);
    const toolCall: ToolCallInfo = {
      id: chunk.id,
      name: chunk.name,
      input: chunk.input,
  ...
  private async handleAgentOutputToolResult(
    ...
    const wasHandled = isLinked || handled !== undefined;
    if (wasHandled) {
      this.seenAgentOutputToolIds.delete(chunk.id);
    }
    return wasHandled;
  ...
  showThinkingIndicator(overrideText?: string, overrideCls?: string): void {
    ...
    state.thinkingIndicatorTimeout = setTimeout(() => {
      ...
      const timerSpan = state.thinkingEl.createSpan({ cls: 'clian-thinking-hint' });
      const updateTimer = () => {
      ...
      state.queueIndicatorEl = state.thinkingEl.createDiv({ cls: 'clian-queue-indicator' });
      this.deps.updateQueueIndicator();
  ...
  private renderCompactBoundary(): void {
    ...
    if (!state.currentContentEl) return;
    this.hideThinkingIndicator();
    const el = state.currentContentEl.createDiv({ cls: 'clian-compact-boundary' });
    el.createSpan({ cls: 'clian-compact-boundary-label', text: 'Conversation compacted' });
  }
  ...
  resetStreamingState(): void {
    const { state } = this.deps;
    this.hideThinkingIndicator();
    this.seenAgentOutputToolIds.clear();
