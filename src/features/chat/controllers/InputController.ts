...
import type { ApprovalCallbackOptions, ChatService } from '../../../core/agent';
...
export class InputController {
  ...
  async sendMessage(options?: {
    ...
    if (state.isStreaming) {
      ...
      if (shouldUseInput) {
        fileContextManager?.clearDroppedFiles?.();
      }
    ...
    const contentEl = msgEl.querySelector('.clian-message-content') as HTMLElement;
    ...
    streamController.showThinkingIndicator(
      ...
      isCompact ? 'clian-thinking--compact' : undefined,
    );
    ...
    try {
      ...
      if (!wasInvalidated && state.streamGeneration === streamGeneration) {
        ...
        if (didCancelThisTurn && !state.pendingNewSessionPlan) {
          await streamController.appendText('\n\n<span class="clian-interrupted">Interrupted</span> <span class="clian-interrupted-hint">· What should Clian do instead?</span>');
        }
        streamController.hideThinkingIndicator();
        state.isStreaming = false;
        state.cancelRequested = false;
  ...
  private async triggerTitleGeneration(): Promise<void> {
    ...
    const activeService = this.getAgentService();
    const flavor = activeService?.flavor ?? 'claude';

    if (!state.currentConversationId) {
      const sessionId = activeService?.getSessionId() ?? undefined;
      const conversation = await plugin.createConversation(sessionId, flavor);
  ...
  async handleApprovalRequest(
    ...
    const headerEl = parentEl.createDiv({ cls: 'clian-ask-approval-info' });
    headerEl.remove();

    const toolEl = headerEl.createDiv({ cls: 'clian-ask-approval-tool' });
    const iconEl = toolEl.createSpan({ cls: 'clian-ask-approval-icon' });
    iconEl.setAttribute('aria-hidden', 'true');
    setToolIcon(iconEl, toolName);
    toolEl.createSpan({ text: toolName, cls: 'clian-ask-approval-tool-name' });

    if (approvalOptions?.decisionReason) {
      headerEl.createDiv({ text: approvalOptions.decisionReason, cls: 'clian-ask-approval-reason' });
    }
    if (approvalOptions?.blockedPath) {
      headerEl.createDiv({ text: approvalOptions.blockedPath, cls: 'clian-ask-approval-blocked-path' });
    }
    if (approvalOptions?.agentID) {
      headerEl.createDiv({ text: `Agent: ${approvalOptions.agentID}`, cls: 'clian-ask-approval-agent' });
    }

    headerEl.createDiv({ text: description, cls: 'clian-ask-approval-desc' });
