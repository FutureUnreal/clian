...
import type { ChatFlavor } from '../../../core/types';
 ...
 *
 * Use a generous default so engine buttons can freely spawn tabs
 * without immediately hitting an artificial low ceiling.
 */
...
/**
 * Minimal interface for the ClianView methods used by TabManager and Tab.
 * Extends Component for Obsidian integration (event handling, cleanup).
 * Avoids circular dependency by not importing ClianView directly.
 */
...
export interface TabUIComponents {
  ...
  thinkingBudgetSelector: ThinkingBudgetSelector | OptionSelector | null;
...
export interface TabData {
  ...
  /** Which backend this tab runs (Claude/Codex/Gemini). */
  flavor: ChatFlavor;
