...
import type { ChatFlavor, SlashCommand } from '../../../core/types';
import { t } from '../../../i18n';
...
export class TabManager implements TabManagerInterface {
   ...
   * @param flavor Optional backend for new/empty tabs.
   * @returns The created tab, or null if max tabs reached.
   */
  async createTab(conversationId?: string | null, tabId?: TabId, flavor?: ChatFlavor): Promise<TabData | null> {
    ...
    const tab = createTab({
      ...
      flavor: conversation?.flavor ?? flavor ?? this.getActiveTab()?.flavor,
      tabId,
  ...
  async restoreState(state: PersistedTabManagerState): Promise<void> {
    ...
    for (const tabState of state.openTabs) {
      try {
        await this.createTab(tabState.conversationId, tabState.tabId, tabState.flavor);
      } catch {
  ...
  async getSdkCommands(): Promise<SlashCommand[]> {
    // Find any ready Claude tab (only Claude exposes SDK slash commands).
    for (const tab of this.tabs.values()) {
      if (tab.service?.flavor === 'claude' && tab.service.isReady()) {
  ...
  /**
   * Broadcasts a function call to all tabs' service instances.
   * Used by settings managers to apply configuration changes to all tabs.
   * @param fn Function to call on each service.
   */
  async broadcastToAllTabs(fn: (service: ChatService) => Promise<void>): Promise<void> {
