...
import { type ChatService, ClianService } from '../../../core/agent';
import { CodexCliService } from '../../../core/agent/CodexCliService';
import { GeminiCliService } from '../../../core/agent/GeminiCliService';
import type { McpServerManager } from '../../../core/mcp';
import type { ChatFlavor, ChatMessage, ClaudeModel, Conversation, PermissionMode, SlashCommand, ThinkingBudget } from '../../../core/types';
import {
...
export interface TabCreateOptions {
  ...
  /** Preferred backend for this tab when no conversation is loaded yet. */
  flavor?: ChatFlavor;
  tabId?: TabId;
...
export function createTab(options: TabCreateOptions): TabData {
  ...
  const contentEl = containerEl.createDiv({ cls: 'clian-tab-content' });
  ...
  const resolvedFlavor: ChatFlavor = flavor ?? conversation?.flavor ?? 'claude';
  contentEl.addClass(`clian-flavor-${resolvedFlavor}`);
  const tab: TabData = {
    id,
...
function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  ...
  const viewHeight = textarea.closest('.clian-container')?.clientHeight ?? window.innerHeight;
...
function buildTabDOM(contentEl: HTMLElement): TabDOMElements {
  ...
  const messagesWrapperEl = contentEl.createDiv({ cls: 'clian-messages-wrapper' });

  // Messages area (inside wrapper)
  const messagesEl = messagesWrapperEl.createDiv({ cls: 'clian-messages' });

  // Welcome message placeholder
  const welcomeEl = messagesEl.createDiv({ cls: 'clian-welcome' });

  // Status panel container (fixed between messages and input)
  const statusPanelContainerEl = contentEl.createDiv({ cls: 'clian-status-panel-container' });

  // Input container
  const inputContainerEl = contentEl.createDiv({ cls: 'clian-input-container' });

  // Nav row (for tab badges and header icons, populated by ClianView)
  const navRowEl = inputContainerEl.createDiv({ cls: 'clian-input-nav-row' });

  const inputWrapper = inputContainerEl.createDiv({ cls: 'clian-input-wrapper' });

  // Context row inside input wrapper (file chips + selection indicator)
  const contextRowEl = inputWrapper.createDiv({ cls: 'clian-context-row' });
...
/**
 * Initializes the tab's ClianService (lazy initialization).
...
export async function initializeTabService(
  ...
  let createdService: ChatService | null = null;
  let unsubscribeReadyState: (() => void) | null = null;

  try {
    // Create per-tab service based on tab flavor
    createdService = tab.flavor === 'codex'
      ? new CodexCliService(plugin)
      : tab.flavor === 'gemini'
        ? new GeminiCliService(plugin)
        : new ClianService(plugin, mcpManager);

    if (!createdService) {
      throw new Error('Failed to create agent service');
    }
    ...
    createdService?.closePersistentQuery('initialization failed');
    tab.service = null;
 ...
 */
function initializeContextManagers(tab: TabData, plugin: ClianPlugin): void {
  const { dom } = tab;
  ...
  tab.ui.imageContextManager = new ImageContextManager(
    ...
    dom.inputEl,
    {
      ...
      onFilesDropped: async (files, event) => {
        await tab.ui.fileContextManager?.handleDroppedFiles(files, event);
      },
    },
    dom.contextRowEl
  );
 ...
 */
function initializeInstructionAndTodo(tab: TabData, plugin: ClianPlugin): void {
  const { dom } = tab;
 ...
 */
function initializeInputToolbar(tab: TabData, plugin: ClianPlugin): void {
  const { dom } = tab;

  const getCodexModelOptions = (): { value: string; label: string; description: string }[] => {
    const vaultPath = getVaultPath(plugin.app);
    const fallback = [...DEFAULT_CODEX_MODELS];
    if (!vaultPath) return fallback;

    const cachePath = path.join(vaultPath, '.codex', 'models_cache.json');
    try {
      ...
      const models = (parsed as any).models;
      ...
      const items = models
        .map((m: any) => ({
          value: typeof m?.slug === 'string' ? m.slug : (typeof m?.id === 'string' ? m.id : ''),
          ...
          show: m?.show_in_picker === undefined ? true : !!m.show_in_picker,
        }))
        .filter((o: any) => typeof o.value === 'string' && o.value.trim())
        .filter((o: any) => o.show)
      ...
      const unique = new Map<string, { value: string; label: string; description: string }>();
      for (const opt of items) {
        if (!unique.has(opt.value)) unique.set(opt.value, opt);
      }
      ...
      const current = (plugin.settings.codexModel || '').trim();
      if (current && !unique.has(current)) {
        out.splice(1, 0, { value: current, label: current, description: 'Custom (from settings)' });
      }
  ...
  const getGeminiModelOptions = (): { value: string; label: string; description: string }[] => {
    const base = [...DEFAULT_GEMINI_MODELS];
    const current = (plugin.settings.geminiModel || '').trim();
  ...
  const inputToolbar = dom.inputWrapper.createDiv({ cls: 'clian-input-toolbar' });
  ...
  const commonCallbacks = {
    ...
    onPermissionModeChange: async (mode: PermissionMode) => {
      ...
      dom.inputWrapper.toggleClass('clian-input-plan-mode', mode === 'plan');
    },
  };

  const codexThinkingOptions = CODEX_REASONING_EFFORT_OPTIONS.map((o) => ({
    value: o.value,
    label: o.label,
    title: o.description,
  }));

  const geminiThinkingOptions = GEMINI_THINKING_MODE_OPTIONS.map((o) => ({
    value: o.value,
    label: o.label,
    title: o.budget === null ? 'Use Gemini defaults' : `thinkingBudget=${o.budget}`,
  }));

  const toolbarComponents = tab.flavor === 'codex'
    ? createInputToolbar(
      inputToolbar,
      {
        ...commonCallbacks,
        ...
        getModelOptions: getCodexModelOptions,
        onModelChange: async (model: ClaudeModel) => {
          const nextModel = String(model || '');
          plugin.settings.codexModel = nextModel;
          await plugin.saveSettings();
          ...
          if (currentUsage) {
            const newContextWindow = getCodexContextWindowSize(nextModel, plugin.settings.customContextLimits);
        ...
        onThinkingBudgetChange: async (_budget: ThinkingBudget) => { },
      },
      {
          optionSelector: {
            labelText: 'Thinking:',
            options: codexThinkingOptions,
            getValue: () => normalizeCodexReasoningEffort(plugin.settings.codexReasoningEffort),
            onChange: async (value: string) => {
              plugin.settings.codexReasoningEffort = normalizeCodexReasoningEffort(value);
              await plugin.saveSettings();
          },
        },
      }
    ...
    : tab.flavor === 'gemini'
      ? createInputToolbar(
        inputToolbar,
        {
          ...
          getModelOptions: getGeminiModelOptions,
          onModelChange: async (model: ClaudeModel) => {
            const nextModel = String(model || '');
            plugin.settings.geminiModel = nextModel;
            await plugin.saveSettings();
            tab.ui.modelSelector?.updateDisplay();
            tab.ui.modelSelector?.renderOptions();
            ...
            const currentUsage = tab.state.usage;
            if (currentUsage) {
              const newContextWindow = getGeminiContextWindowSize(nextModel, plugin.settings.customContextLimits);
              const newPercentage = Math.min(100, Math.max(0, Math.round((currentUsage.contextTokens / newContextWindow) * 100)));
              tab.state.usage = {
                ...currentUsage,
                model: nextModel || undefined,
        ...
        },
        {
          optionSelector: {
            ...
            getValue: () => normalizeGeminiThinkingMode(plugin.settings.geminiThinkingMode),
            onChange: async (value: string) => {
              plugin.settings.geminiThinkingMode = normalizeGeminiThinkingMode(value);
              await plugin.saveSettings();
            },
          },
        }
      )
      : createInputToolbar(inputToolbar, {
        ...commonCallbacks,
        getSettings: () => ({
        ...
        getAdditionalModelOptions: () =>
          (plugin.settings.extraClaudeModels || []).map((modelId) => ({
            value: modelId,
            label: modelId,
  ...
  // Ensure selectors exist for each engine (per-tab toolbar is flavor-specific).
  ...
  dom.inputWrapper.toggleClass('clian-input-plan-mode', plugin.settings.permissionMode === 'plan');
}
...
export function initializeTabUI(
  ...
  dom.selectionIndicatorEl = dom.contextRowEl.createDiv({ cls: 'clian-selection-indicator' });
  dom.selectionIndicatorEl.style.display = 'none';

  // Canvas selection indicator
  dom.canvasIndicatorEl = dom.contextRowEl.createDiv({ cls: 'clian-canvas-indicator' });
 ...
 */
function resolveForkSource(tab: TabData, plugin: ClianPlugin): ForkSource | null {
 ...
 */
export function wireTabInputEvents(tab: TabData, plugin: ClianPlugin): void {
 ...
 */
export function getTabTitle(tab: TabData, plugin: ClianPlugin): string {
  let title = 'New Chat';
  if (tab.conversationId) {
    const conversation = plugin.getConversationSync(tab.conversationId);
  ...
  if (tab.flavor !== 'claude') {
    const label = tab.flavor === 'codex' ? 'Codex' : tab.flavor === 'gemini' ? 'Gemini' : tab.flavor;
    return `${label}: ${title}`;
  }
...
export function setupServiceCallbacks(tab: TabData, plugin: ClianPlugin): void {
...
export function updatePlanModeUI(tab: TabData, plugin: ClianPlugin, mode: PermissionMode): void {
  plugin.settings.permissionMode = mode;
  void plugin.saveSettings();
  tab.ui.permissionToggle?.updateDisplay();
  tab.dom.inputWrapper.toggleClass('clian-input-plan-mode', mode === 'plan');
}
