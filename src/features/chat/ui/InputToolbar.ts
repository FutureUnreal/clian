...
export interface ToolbarCallbacks {
  ...
  /** Optional model options override (used for non-Claude engines). */
  getModelOptions?: () => { value: string; label: string; description: string }[];
  /** Optional extra models to merge into the picker (e.g. user-defined Claude model IDs). */
  getAdditionalModelOptions?: () => { value: string; label: string; description: string }[];
}
...
export class ModelSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    ...
    this.container = parentEl.createDiv({ cls: 'clian-model-selector' });
    this.render();
  }
  ...
  private getAvailableModels() {
    ...
    if (this.callbacks.getModelOptions) {
      const provided = this.callbacks.getModelOptions();
      models = Array.isArray(provided) ? [...provided] : [];
    } else {
    ...
    if (this.callbacks.getAdditionalModelOptions) {
      const additional = this.callbacks.getAdditionalModelOptions();
    ...
    // When 1M context is enabled, annotate Sonnet models with "(1M)" for clarity.
    const settings = this.callbacks.getSettings();
    if (settings.show1MModel) {
      models = models.map((m) => {
        const isSonnet = typeof m.value === 'string' && m.value.toLowerCase().includes('sonnet');
        if (!isSonnet) return m;

        if (m.label.includes('1M')) return m;
        const nextLabel = m.label.endsWith('(Alias)')
          ? m.label.replace(/\(Alias\)$/, '(Alias, 1M)')
          : `${m.label} (1M)`;

        return { ...m, label: nextLabel };
      });
    }
  ...
  private render() {
    ...
    this.buttonEl = this.container.createDiv({ cls: 'clian-model-btn' });
    this.setReady(this.isReady);
    this.updateDisplay();

    this.dropdownEl = this.container.createDiv({ cls: 'clian-model-dropdown' });
    this.renderOptions();
  }
  ...
  updateDisplay() {
    ...
    const labelEl = this.buttonEl.createSpan({ cls: 'clian-model-label' });
  ...
  /** Hide/show the selector (used for non-Claude flavors). */
  ...
  renderOptions() {
    ...
    for (const model of [...models].reverse()) {
      const option = this.dropdownEl.createDiv({ cls: 'clian-model-option' });
...
export class ThinkingBudgetSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'clian-thinking-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    const labelEl = this.container.createSpan({ cls: 'clian-thinking-label-text' });
    labelEl.setText('Thinking:');

    this.gearsEl = this.container.createDiv({ cls: 'clian-thinking-gears' });
    this.renderGears();
  }
  ...
  private renderGears() {
    ...
    const currentEl = this.gearsEl.createDiv({ cls: 'clian-thinking-current' });
    currentEl.setText(currentBudgetInfo?.label || 'Off');

    const optionsEl = this.gearsEl.createDiv({ cls: 'clian-thinking-options' });

    for (const budget of [...THINKING_BUDGETS].reverse()) {
      const gearEl = optionsEl.createDiv({ cls: 'clian-thinking-gear' });
...
export type OptionSelectorOption = { value: string; label: string; title?: string };
...
export class OptionSelector {
  private container: HTMLElement;
  private gearsEl: HTMLElement | null = null;
  private config: OptionSelectorConfig;

  constructor(parentEl: HTMLElement, config: OptionSelectorConfig) {
    this.config = config;
    this.container = parentEl.createDiv({ cls: 'clian-thinking-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    const labelEl = this.container.createSpan({ cls: 'clian-thinking-label-text' });
    labelEl.setText(this.config.labelText);
  ...
  private renderGears() {
    ...
    const currentInfo = this.config.options.find((o) => o.value === currentValue) ?? this.config.options[0];

    const currentEl = this.gearsEl.createDiv({ cls: 'clian-thinking-current' });
    currentEl.setText(currentInfo?.label || 'Auto');

    const optionsEl = this.gearsEl.createDiv({ cls: 'clian-thinking-options' });

    for (const opt of [...this.config.options].reverse()) {
      const gearEl = optionsEl.createDiv({ cls: 'clian-thinking-gear' });
      gearEl.setText(opt.label);
      if (opt.title) {
        gearEl.setAttribute('title', opt.title);
      }
      ...
      gearEl.addEventListener('click', async (e) => {
        e.stopPropagation();
        await this.config.onChange(opt.value);
        this.updateDisplay();
      });
    }
  }
...
export class PermissionToggle {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'clian-permission-toggle' });
    this.render();
  }
  ...
  private render() {
    ...
    this.labelEl = this.container.createSpan({ cls: 'clian-permission-label' });
    this.toggleEl = this.container.createDiv({ cls: 'clian-toggle-switch' });
...
export class ExternalContextSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'clian-external-context-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    const iconWrapper = this.container.createDiv({ cls: 'clian-external-context-icon-wrapper' });

    this.iconEl = iconWrapper.createDiv({ cls: 'clian-external-context-icon' });
    setIcon(this.iconEl, 'folder');

    this.badgeEl = iconWrapper.createDiv({ cls: 'clian-external-context-badge' });
    ...
    this.dropdownEl = this.container.createDiv({ cls: 'clian-external-context-dropdown' });
    this.renderDropdown();
  }
  ...
  private renderDropdown() {
    ...
    // Header
    const headerEl = this.dropdownEl.createDiv({ cls: 'clian-external-context-header' });
    headerEl.setText('External Contexts');

    // Path list
    const listEl = this.dropdownEl.createDiv({ cls: 'clian-external-context-list' });

    if (this.externalContextPaths.length === 0) {
      const emptyEl = listEl.createDiv({ cls: 'clian-external-context-empty' });
      emptyEl.setText('Click folder icon to add');
    } else {
      for (const pathStr of this.externalContextPaths) {
        const itemEl = listEl.createDiv({ cls: 'clian-external-context-item' });

        const pathTextEl = itemEl.createSpan({ cls: 'clian-external-context-text' });
        ...
        const lockBtn = itemEl.createSpan({ cls: 'clian-external-context-lock' });
        if (isPersistent) {
        ...
        const removeBtn = itemEl.createSpan({ cls: 'clian-external-context-remove' });
        setIcon(removeBtn, 'x');
...
export class McpServerSelector {
  ...
  constructor(parentEl: HTMLElement) {
    this.container = parentEl.createDiv({ cls: 'clian-mcp-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    const iconWrapper = this.container.createDiv({ cls: 'clian-mcp-selector-icon-wrapper' });

    this.iconEl = iconWrapper.createDiv({ cls: 'clian-mcp-selector-icon' });
    this.iconEl.innerHTML = MCP_ICON_SVG;

    this.badgeEl = iconWrapper.createDiv({ cls: 'clian-mcp-selector-badge' });

    this.updateDisplay();

    this.dropdownEl = this.container.createDiv({ cls: 'clian-mcp-selector-dropdown' });
    this.renderDropdown();
  ...
  private renderDropdown() {
    ...
    // Header
    const headerEl = this.dropdownEl.createDiv({ cls: 'clian-mcp-selector-header' });
    headerEl.setText('MCP Servers');

    // Server list
    const listEl = this.dropdownEl.createDiv({ cls: 'clian-mcp-selector-list' });

    const allServers = this.mcpManager?.getServers() || [];
    const servers = allServers.filter(s => s.enabled);

    if (servers.length === 0) {
      const emptyEl = listEl.createDiv({ cls: 'clian-mcp-selector-empty' });
  ...
  private renderServerItem(listEl: HTMLElement, server: ClianMcpServer) {
    const itemEl = listEl.createDiv({ cls: 'clian-mcp-selector-item' });
    ...
    // Checkbox
    const checkEl = itemEl.createDiv({ cls: 'clian-mcp-selector-check' });
    if (isEnabled) {
      checkEl.innerHTML = CHECK_ICON_SVG;
    }

    // Info
    const infoEl = itemEl.createDiv({ cls: 'clian-mcp-selector-item-info' });

    const nameEl = infoEl.createSpan({ cls: 'clian-mcp-selector-item-name' });
    nameEl.setText(server.name);

    // Badges
    if (server.contextSaving) {
      const csEl = infoEl.createSpan({ cls: 'clian-mcp-selector-cs-badge' });
      csEl.setText('@');
  ...
  private toggleServer(name: string, itemEl: HTMLElement) {
    ...
    const checkEl = itemEl.querySelector('.clian-mcp-selector-check') as HTMLElement | null;
...
export class ContextUsageMeter {
  ...
  constructor(parentEl: HTMLElement) {
    this.container = parentEl.createDiv({ cls: 'clian-context-meter' });
    this.render();
    // Initially hidden
    this.container.style.display = 'none';
  }
  ...
  private render() {
    ...
    const gaugeEl = this.container.createDiv({ cls: 'clian-context-meter-gauge' });
    gaugeEl.innerHTML = `
      <svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
        <path class="clian-meter-bg"
          d="M ${x1} ${y1} A ${radius} ${radius} 0 1 1 ${x2} ${y2}"
          fill="none" stroke-width="${strokeWidth}" stroke-linecap="round"/>
        <path class="clian-meter-fill"
          d="M ${x1} ${y1} A ${radius} ${radius} 0 1 1 ${x2} ${y2}"
          fill="none" stroke-width="${strokeWidth}" stroke-linecap="round"
...
export function createInputToolbar(
  ...
  options?: { optionSelector?: OptionSelectorConfig }
): {
  modelSelector: ModelSelector;
  thinkingBudgetSelector: ThinkingBudgetSelector | OptionSelector;
  ...
  const thinkingBudgetSelector = options?.optionSelector
    ? new OptionSelector(parentEl, options.optionSelector)
