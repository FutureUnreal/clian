...
export class NavigationSidebar {
  ...
  constructor(
    ...
    this.container = this.parentEl.createDiv({ cls: 'clian-nav-sidebar' });

    // Create buttons
    this.topBtn = this.createButton('clian-nav-btn-top', 'chevrons-up', 'Scroll to top');
    this.prevBtn = this.createButton('clian-nav-btn-prev', 'chevron-up', 'Previous message');
    this.nextBtn = this.createButton('clian-nav-btn-next', 'chevron-down', 'Next message');
    this.bottomBtn = this.createButton('clian-nav-btn-bottom', 'chevrons-down', 'Scroll to bottom');
  ...
  private createButton(cls: string, icon: string, label: string): HTMLElement {
    const btn = this.container.createDiv({ cls: `clian-nav-btn ${cls}` });
    setIcon(btn, icon);
  ...
  private scrollToMessage(direction: 'prev' | 'next'): void {
    const messages = Array.from(this.messagesEl.querySelectorAll('.clian-message-user')) as HTMLElement[];
