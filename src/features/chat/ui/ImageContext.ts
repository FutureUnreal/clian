...
export interface ImageContextCallbacks {
  ...
  onFilesDropped?: (files: File[], event: DragEvent) => void | Promise<void>;
}
...
export class ImageContextManager {
  ...
  constructor(
    ...
    const fileIndicator = this.previewContainerEl.querySelector('.clian-file-indicator');
    this.imagePreviewEl = this.previewContainerEl.createDiv({ cls: 'clian-image-preview' });
  ...
  private setupDragAndDrop() {
    const inputWrapper = this.containerEl.querySelector('.clian-input-wrapper') as HTMLElement;
    if (!inputWrapper) return;

    this.dropOverlay = inputWrapper.createDiv({ cls: 'clian-drop-overlay' });
    const dropContent = this.dropOverlay.createDiv({ cls: 'clian-drop-content' });
    ...
    dropContent.createSpan({ text: 'Drop files or images here' });
  ...
  private handleDragLeave(e: DragEvent) {
    ...
    const inputWrapper = this.containerEl.querySelector('.clian-input-wrapper');
    if (!inputWrapper) {
  ...
  private async handleDrop(e: DragEvent) {
    ...
    if (nonImageFiles.length > 0 || files.length === 0) {
      await this.callbacks.onFilesDropped?.(nonImageFiles, e);
    }
  }
  ...
  private hasDraggedFiles(e: DragEvent): boolean {
    ...
    if (typeof (types as { includes?: (value: string) => boolean }).includes === 'function') {
      return (types as { includes: (value: string) => boolean }).includes('Files');
    }
  ...
  private renderImagePreview(id: string, image: ImageAttachment) {
    const previewEl = this.imagePreviewEl.createDiv({ cls: 'clian-image-chip' });

    const thumbEl = previewEl.createDiv({ cls: 'clian-image-thumb' });
    ...
    const infoEl = previewEl.createDiv({ cls: 'clian-image-info' });
    const nameEl = infoEl.createSpan({ cls: 'clian-image-name' });
    nameEl.setText(this.truncateName(image.name, 20));
    nameEl.setAttribute('title', image.name);

    const sizeEl = infoEl.createSpan({ cls: 'clian-image-size' });
    sizeEl.setText(this.formatSize(image.size));

    const removeEl = previewEl.createSpan({ cls: 'clian-image-remove' });
    removeEl.setText('\u00D7');
