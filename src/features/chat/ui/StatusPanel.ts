...
export class StatusPanel {
  ...
  private createPanel(): void {
    ...
    this.bashOutputContainerEl.className = 'clian-status-panel-bash';
    this.bashOutputContainerEl.style.display = 'none';

    this.bashHeaderEl = document.createElement('div');
    this.bashHeaderEl.className = 'clian-tool-header clian-status-panel-bash-header';
    ...
    this.bashContentEl.className = 'clian-status-panel-bash-content';
    ...
    this.todoContainerEl.className = 'clian-status-panel-todos';
    ...
    this.todoHeaderEl.className = 'clian-status-panel-header';
    ...
    this.todoContentEl.className = 'clian-status-panel-content clian-todo-list-container';
  ...
  private renderTodoHeader(completedCount: number, totalCount: number, currentTask: TodoItem | undefined): void {
    ...
    if (!this.isTodoExpanded) {
      ...
      if (completedCount === totalCount && totalCount > 0) {
        ...
        status.className = 'clian-status-panel-status status-completed';
      ...
      if (currentTask) {
        ...
        current.className = 'clian-status-panel-current';
  ...
  private renderSubagentStatus(): void {
    ...
    for (let i = 0; i < completedSubagents.length; i++) {
      ...
      rowEl.className = showRunningOnThisRow
        ? 'clian-status-panel-done-row clian-status-panel-combined-row'
        : 'clian-status-panel-done-row';

      // Bot icon
      const botIconEl = document.createElement('span');
      botIconEl.className = 'clian-status-panel-icon clian-status-panel-bot-icon';
      setIcon(botIconEl, getToolIcon(TOOL_TASK));
      rowEl.appendChild(botIconEl);

      // Description text
      const textEl = document.createElement('span');
      textEl.className = 'clian-status-panel-done-text';
      textEl.textContent = this.truncateDescription(subagent.description);
      rowEl.appendChild(textEl);

      // Green tick icon
      const iconEl = document.createElement('span');
      iconEl.className = 'clian-status-panel-icon clian-status-panel-done-icon';
      ...
      if (showRunningOnThisRow) {
        ...
        runningEl.className = 'clian-status-panel-running-text';
    ...
    if (runningSubagents.length > 0 && completedSubagents.length === 0) {
      const rowEl = document.createElement('div');
      rowEl.className = 'clian-status-panel-running-row';

      // Count text
      const textEl = document.createElement('span');
      textEl.className = 'clian-status-panel-running-text';
  ...
  private renderBashOutputs(options: { scroll?: boolean } = {}): void {
    ...
    headerIconEl.className = 'clian-tool-icon';
    ...
    headerLabelEl.className = 'clian-tool-label';
    ...
    previewEl.className = 'clian-tool-current';
    ...
    summaryStatusEl.className = 'clian-tool-status';
    ...
    actionsEl.className = 'clian-status-panel-bash-actions';
  ...
  private renderBashEntry(info: PanelBashOutput): HTMLElement {
    ...
    entryLabelEl.className = 'clian-tool-label';
    ...
    entryStatusEl.className = 'clian-tool-status';
    ...
    contentEl.className = 'clian-tool-content';
    ...
    const rowEl = document.createElement('div');
    rowEl.className = 'clian-tool-result-row';

    const textEl = document.createElement('span');
    textEl.className = 'clian-tool-result-text';
  ...
  private appendActionButton(
    ...
    el.className = `clian-status-panel-bash-action clian-status-panel-bash-action-${name}`;
