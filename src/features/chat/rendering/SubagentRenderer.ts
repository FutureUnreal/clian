...
function createSection(parentEl: HTMLElement, title: string, bodyClass?: string): SubagentSection {
  const wrapperEl = parentEl.createDiv({ cls: 'clian-subagent-section' });

  const headerEl = wrapperEl.createDiv({ cls: 'clian-subagent-section-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const titleEl = headerEl.createDiv({ cls: 'clian-subagent-section-title' });
  titleEl.setText(title);

  const bodyEl = wrapperEl.createDiv({ cls: 'clian-subagent-section-body' });
...
function setPromptText(promptBodyEl: HTMLElement, prompt: string): void {
  promptBodyEl.empty();
  const textEl = promptBodyEl.createDiv({ cls: 'clian-subagent-prompt-text' });
...
function renderSubagentToolContent(contentEl: HTMLElement, toolCall: ToolCallInfo): void {
  ...
  if (!toolCall.result) {
    const emptyEl = contentEl.createDiv({ cls: 'clian-subagent-tool-empty' });
...
function setSubagentToolStatus(view: SubagentToolView, status: ToolCallInfo['status']): void {
  view.statusEl.className = 'clian-subagent-tool-status';
...
function updateSubagentToolView(view: SubagentToolView, toolCall: ToolCallInfo): void {
  view.wrapperEl.className = `clian-subagent-tool-item clian-subagent-tool-${toolCall.status}`;
...
function createSubagentToolView(parentEl: HTMLElement, toolCall: ToolCallInfo): SubagentToolView {
  const wrapperEl = parentEl.createDiv({
    cls: `clian-subagent-tool-item clian-subagent-tool-${toolCall.status}`,
  });
  wrapperEl.dataset.toolId = toolCall.id;

  const headerEl = wrapperEl.createDiv({ cls: 'clian-subagent-tool-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const iconEl = headerEl.createDiv({ cls: 'clian-subagent-tool-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setToolIcon(iconEl, toolCall.name);

  const nameEl = headerEl.createDiv({ cls: 'clian-subagent-tool-name' });
  const summaryEl = headerEl.createDiv({ cls: 'clian-subagent-tool-summary' });
  const statusEl = headerEl.createDiv({ cls: 'clian-subagent-tool-status' });

  const contentEl = wrapperEl.createDiv({ cls: 'clian-subagent-tool-content' });
...
function ensureResultSection(state: SubagentState): SubagentSection {
  ...
  const section = createSection(state.contentEl, 'Result', 'clian-subagent-result-body');
  section.wrapperEl.addClass('clian-subagent-section-result');
...
function setResultText(state: SubagentState, text: string): void {
  ...
  const resultEl = section.bodyEl.createDiv({ cls: 'clian-subagent-result-output' });
...
function hydrateSyncSubagentStateFromStored(state: SubagentState, subagent: SubagentInfo): void {
  ...
  if (subagent.status === 'completed' || subagent.status === 'error') {
    ...
    state.statusEl.className = 'clian-subagent-status status-running';
...
export function createSubagentBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'clian-subagent-list' });
  wrapperEl.dataset.subagentId = taskToolId;

  const headerEl = wrapperEl.createDiv({ cls: 'clian-subagent-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const iconEl = headerEl.createDiv({ cls: 'clian-subagent-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setIcon(iconEl, getToolIcon(TOOL_TASK));

  const labelEl = headerEl.createDiv({ cls: 'clian-subagent-label' });
  labelEl.setText(truncateDescription(description));

  const countEl = headerEl.createDiv({ cls: 'clian-subagent-count' });
  countEl.setText('0 tool uses');

  const statusEl = headerEl.createDiv({ cls: 'clian-subagent-status status-running' });
  statusEl.setAttribute('aria-label', 'Status: running');

  const contentEl = wrapperEl.createDiv({ cls: 'clian-subagent-content' });

  const promptSection = createSection(contentEl, 'Prompt', 'clian-subagent-prompt-body');
  promptSection.wrapperEl.addClass('clian-subagent-section-prompt');
  setPromptText(promptSection.bodyEl, prompt);

  const toolsContainerEl = contentEl.createDiv({ cls: 'clian-subagent-tools' });
...
export function finalizeSubagentBlock(
  ...
  state.statusEl.className = 'clian-subagent-status';
...
function renderAsyncContentLikeSync(
  ...
  const resultSection = createSection(contentEl, 'Result', 'clian-subagent-result-body');
  resultSection.wrapperEl.addClass('clian-subagent-section-result');
  const resultEl = resultSection.bodyEl.createDiv({ cls: 'clian-subagent-result-output' });
...
export function createAsyncSubagentBlock(
  ...
  const statusTextEl = headerEl.createDiv({ cls: 'clian-subagent-status-text' });
...
export function markAsyncSubagentOrphaned(state: AsyncSubagentState): void {
  ...
  state.statusEl.className = 'clian-subagent-status status-error';
  state.statusEl.empty();
...
export function renderStoredAsyncSubagent(
  ...
  const statusEl = headerEl.createDiv({ cls: `clian-subagent-status ${statusIconClass}` });
