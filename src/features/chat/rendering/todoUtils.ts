...
export function renderTodoItems(
  ...
  for (const todo of todos) {
    const item = container.createDiv({ cls: `clian-todo-item clian-todo-${todo.status}` });

    const icon = item.createSpan({ cls: 'clian-todo-status-icon' });
    icon.setAttribute('aria-hidden', 'true');
    setIcon(icon, getTodoStatusIcon(todo.status));

    const text = item.createSpan({ cls: 'clian-todo-text' });
