...
function renderWebSearchExpanded(container: HTMLElement, result: string): void {
  ...
  const linksEl = container.createDiv({ cls: 'clian-tool-lines' });
  for (const link of parsed.links) {
    const linkEl = linksEl.createEl('a', { cls: 'clian-tool-link' });
    linkEl.setAttribute('href', link.url);
    linkEl.setAttribute('target', '_blank');
    linkEl.setAttribute('rel', 'noopener noreferrer');

    const iconEl = linkEl.createSpan({ cls: 'clian-tool-link-icon' });
    setIcon(iconEl, 'external-link');

    linkEl.createSpan({ cls: 'clian-tool-link-title', text: link.title });
  }

  if (parsed.summary) {
    const summaryEl = container.createDiv({ cls: 'clian-tool-web-summary' });
...
function renderFileSearchExpanded(container: HTMLElement, result: string): void {
  ...
  if (lines.length === 0) {
    container.createDiv({ cls: 'clian-tool-empty', text: 'No matches found' });
    return;
  }
...
function renderLinesExpanded(
  ...
  const linesEl = container.createDiv({ cls: 'clian-tool-lines' });
  for (const line of displayLines) {
    const stripped = line.replace(/^\s*\d+→/, '');
    const lineEl = linesEl.createDiv({ cls: 'clian-tool-line' });
...
export function renderExpandedContent(container: HTMLElement, toolName: string, result: string | undefined): void {
  if (!result) {
    container.createDiv({ cls: 'clian-tool-empty', text: 'No result' });
    return;
  }
...
function resetStatusElement(statusEl: HTMLElement, statusClass: string, ariaLabel: string): void {
  statusEl.className = 'clian-tool-status';
  statusEl.empty();
...
export function renderTodoWriteResult(
  ...
  container.empty();
  container.addClass('clian-todo-panel-content');
  container.addClass('clian-todo-list-container');

  const todos = input.todos as TodoItem[] | undefined;
  if (!todos || !Array.isArray(todos)) {
    const item = container.createSpan({ cls: 'clian-tool-result-item' });
...
function createToolElementStructure(
  ...
  const toolEl = parentEl.createDiv({ cls: 'clian-tool-call' });

  const header = toolEl.createDiv({ cls: 'clian-tool-header' });
  header.setAttribute('tabindex', '0');
  header.setAttribute('role', 'button');

  const iconEl = header.createSpan({ cls: 'clian-tool-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setToolIcon(iconEl, toolCall.name);

  const nameEl = header.createSpan({ cls: 'clian-tool-name' });
  nameEl.setText(getToolName(toolCall.name, toolCall.input));

  const summaryEl = header.createSpan({ cls: 'clian-tool-summary' });
  ...
  const statusEl = header.createSpan({ cls: 'clian-tool-status' });

  const content = toolEl.createDiv({ cls: 'clian-tool-content' });
...
function renderAskUserQuestionResult(container: HTMLElement, toolCall: ToolCallInfo): boolean {
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    bodyEl.createDiv({
      ...
      cls: answer ? 'clian-ask-review-a-text' : 'clian-ask-review-empty',
    });
  }
...
function contentFallback(container: HTMLElement, text: string): void {
  const resultRow = container.createDiv({ cls: 'clian-tool-result-row' });
  const resultText = resultRow.createSpan({ cls: 'clian-tool-result-text' });
...
function createCurrentTaskPreview(
  ...
  const currentTaskEl = header.createSpan({ cls: 'clian-tool-current' });
...
export function updateToolCallResult(
  ...
  if (toolCall.name === TOOL_TODO_WRITE) {
    const statusEl = toolEl.querySelector('.clian-tool-status') as HTMLElement;
    if (statusEl) {
      setTodoWriteStatus(statusEl, toolCall.input);
    }
    const content = toolEl.querySelector('.clian-tool-content') as HTMLElement;
    if (content) {
      renderTodoWriteResult(content, toolCall.input);
    }
    const nameEl = toolEl.querySelector('.clian-tool-name') as HTMLElement;
    if (nameEl) {
      nameEl.setText(getToolName(toolCall.name, toolCall.input));
    }
    const currentTaskEl = toolEl.querySelector('.clian-tool-current') as HTMLElement;
    if (currentTaskEl) {
