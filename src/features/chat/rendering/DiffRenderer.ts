...
export function renderDiffContent(
  ...
  if (allInserts && diffLines.length > NEW_FILE_DISPLAY_CAP) {
    const hunkEl = containerEl.createDiv({ cls: 'clian-diff-hunk' });
    for (const line of diffLines.slice(0, NEW_FILE_DISPLAY_CAP)) {
      const lineEl = hunkEl.createDiv({ cls: 'clian-diff-line clian-diff-insert' });
      const prefixEl = lineEl.createSpan({ cls: 'clian-diff-prefix' });
      prefixEl.setText('+');
      const contentEl = lineEl.createSpan({ cls: 'clian-diff-text' });
      contentEl.setText(line.text || ' ');
    }
    const remaining = diffLines.length - NEW_FILE_DISPLAY_CAP;
    const separator = containerEl.createDiv({ cls: 'clian-diff-separator' });
  ...
  if (hunks.length === 0) {
    // No changes
    const noChanges = containerEl.createDiv({ cls: 'clian-diff-no-changes' });
  ...
  hunks.forEach((hunk, hunkIndex) => {
    ...
    for (const line of hunk.lines) {
      const lineEl = hunkEl.createDiv({ cls: `clian-diff-line clian-diff-${line.type}` });
