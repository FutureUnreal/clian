...
export class MessageRenderer {
  ...
  addMessage(msg: ChatMessage): HTMLElement {
    ...
    const msgEl = this.messagesEl.createDiv({
      cls: `clian-message clian-message-${msg.role}`,
      attr: {
    ...
    const contentEl = msgEl.createDiv({ cls: 'clian-message-content', attr: { dir: 'auto' } });

    if (msg.role === 'user') {
      const textToShow = msg.displayContent ?? msg.content;
      if (textToShow) {
        const textEl = contentEl.createDiv({ cls: 'clian-text-block' });
  ...
  renderMessages(
    ...
    const newWelcomeEl = this.messagesEl.createDiv({ cls: 'clian-welcome' });
    newWelcomeEl.createDiv({ cls: 'clian-welcome-greeting', text: getGreeting() });
  ...
  private renderInterruptMessage(): void {
    const msgEl = this.messagesEl.createDiv({ cls: 'clian-message clian-message-assistant' });
    const contentEl = msgEl.createDiv({ cls: 'clian-message-content', attr: { dir: 'auto' } });
    const textEl = contentEl.createDiv({ cls: 'clian-text-block' });
    textEl.innerHTML = '<span class="clian-interrupted">Interrupted</span> <span class="clian-interrupted-hint">· What should Clian do instead?</span>';
  }
  ...
  private renderAssistantContent(msg: ChatMessage, contentEl: HTMLElement): void {
    if (msg.contentBlocks && msg.contentBlocks.length > 0) {
      ...
      for (const block of msg.contentBlocks) {
        if (block.type === 'thinking') {
          ...
          const boundaryEl = contentEl.createDiv({ cls: 'clian-compact-boundary' });
          boundaryEl.createSpan({ cls: 'clian-compact-boundary-label', text: 'Conversation compacted' });
    ...
    if (msg.durationSeconds && msg.durationSeconds > 0 && !hasCompactBoundary) {
      ...
      const footerEl = contentEl.createDiv({ cls: 'clian-response-footer' });
      footerEl.createSpan({
        // Render as metadata (not Markdown) to avoid looking like model output.
        text: `${flavorWord} · ${formatDurationMmSs(msg.durationSeconds)}`,
        cls: 'clian-baked-duration',
      });
    }
  }
  ...
  renderMessageImages(containerEl: HTMLElement, images: ImageAttachment[]): void {
    const imagesEl = containerEl.createDiv({ cls: 'clian-message-images' });

    for (const image of images) {
      const imageWrapper = imagesEl.createDiv({ cls: 'clian-message-image' });
  ...
  showFullImage(image: ImageAttachment): void {
    ...
    const overlay = document.body.createDiv({ cls: 'clian-image-modal-overlay' });
    const modal = overlay.createDiv({ cls: 'clian-image-modal' });
    ...
    const closeBtn = modal.createDiv({ cls: 'clian-image-modal-close' });
    closeBtn.setText('\u00D7');
  ...
  async renderContent(el: HTMLElement, markdown: string): Promise<void> {
    ...
    try {
      ...
      el.querySelectorAll('pre').forEach((pre) => {
        ...
        if (pre.parentElement?.classList.contains('clian-code-wrapper')) return;

        // Create wrapper
        const wrapper = createEl('div', { cls: 'clian-code-wrapper' });
  ...
  addTextCopyButton(textEl: HTMLElement, markdown: string): void {
    const copyBtn = textEl.createSpan({ cls: 'clian-text-copy-btn' });
  ...
  refreshActionButtons(msg: ChatMessage, allMessages?: ChatMessage[], index?: number): void {
    ...
    if (this.rewindCallback && !msgEl.querySelector('.clian-message-rewind-btn')) {
      this.addRewindButton(msgEl, msg.id);
    }
    if (this.forkCallback && !msgEl.querySelector('.clian-message-fork-btn')) {
      this.addForkButton(msgEl, msg.id);
    }
  ...
  private cleanupLiveMessageEl(msgId: string, msgEl: HTMLElement): void {
    const needsRewind = this.rewindCallback && !msgEl.querySelector('.clian-message-rewind-btn');
    const needsFork = this.forkCallback && !msgEl.querySelector('.clian-message-fork-btn');
  ...
  private getOrCreateActionsToolbar(msgEl: HTMLElement): HTMLElement {
    const existing = msgEl.querySelector('.clian-user-msg-actions') as HTMLElement | null;
    if (existing) return existing;
    return msgEl.createDiv({ cls: 'clian-user-msg-actions' });
  }

  private addUserCopyButton(msgEl: HTMLElement, content: string): void {
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const copyBtn = toolbar.createSpan({ cls: 'clian-user-msg-copy-btn' });
    copyBtn.innerHTML = MessageRenderer.COPY_ICON;
  ...
  private addRewindButton(msgEl: HTMLElement, messageId: string): void {
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'clian-message-rewind-btn' });
  ...
  private addForkButton(msgEl: HTMLElement, messageId: string): void {
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'clian-message-fork-btn' });
    if (toolbar.firstChild !== btn) toolbar.insertBefore(btn, toolbar.firstChild);
