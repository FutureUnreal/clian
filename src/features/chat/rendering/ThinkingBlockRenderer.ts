...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'clian-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'clian-thinking-header' });
  ...
  // Label with timer
  const labelEl = header.createSpan({ cls: 'clian-thinking-label' });
  ...
  const contentEl = wrapperEl.createDiv({ cls: 'clian-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  const header = state.wrapperEl.querySelector('.clian-thinking-header');
  if (header) {
