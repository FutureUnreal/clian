...
export class InlineExitPlanMode {
  ...
  render(): void {
    this.rootEl = this.containerEl.createDiv({ cls: 'clian-plan-approval-inline' });

    const titleEl = this.rootEl.createDiv({ cls: 'clian-plan-inline-title' });
    titleEl.setText('Plan complete');

    this.planContent = this.readPlanContent();
    if (this.planContent) {
      const contentEl = this.rootEl.createDiv({ cls: 'clian-plan-content-preview' });
      if (this.renderContent) {
        void this.renderContent(contentEl, this.planContent);
      } else {
        contentEl.createDiv({ cls: 'clian-plan-content-text', text: this.planContent });
      }
    } else if (this.planReadError) {
      this.rootEl.createDiv({
        cls: 'clian-plan-content-preview clian-plan-read-error',
    ...
    if (allowedPrompts && Array.isArray(allowedPrompts) && allowedPrompts.length > 0) {
      const permEl = this.rootEl.createDiv({ cls: 'clian-plan-permissions' });
      permEl.createDiv({ text: 'Requested permissions:', cls: 'clian-plan-permissions-label' });
      const listEl = permEl.createEl('ul', { cls: 'clian-plan-permissions-list' });
      for (const perm of allowedPrompts) {
        listEl.createEl('li', { text: perm.prompt });
      }
    }

    const actionsEl = this.rootEl.createDiv({ cls: 'clian-ask-list' });

    const newSessionRow = actionsEl.createDiv({ cls: 'clian-ask-item' });
    newSessionRow.addClass('is-focused');
    newSessionRow.createSpan({ text: '\u203A', cls: 'clian-ask-cursor' });
    newSessionRow.createSpan({ text: '1. ', cls: 'clian-ask-item-num' });
    newSessionRow.createSpan({ text: 'Approve (new session)', cls: 'clian-ask-item-label' });
    ...
    const approveRow = actionsEl.createDiv({ cls: 'clian-ask-item' });
    approveRow.createSpan({ text: '\u00A0', cls: 'clian-ask-cursor' });
    approveRow.createSpan({ text: '2. ', cls: 'clian-ask-item-num' });
    approveRow.createSpan({ text: 'Approve (current session)', cls: 'clian-ask-item-label' });
    ...
    const feedbackRow = actionsEl.createDiv({ cls: 'clian-ask-item clian-ask-custom-item' });
    feedbackRow.createSpan({ text: '\u00A0', cls: 'clian-ask-cursor' });
    feedbackRow.createSpan({ text: '3. ', cls: 'clian-ask-item-num' });
    ...
    this.rootEl.createDiv({ text: HINTS_TEXT, cls: 'clian-ask-hints' });
