...
export class InlineAskUserQuestion {
  ...
  render(): void {
    this.rootEl = this.containerEl.createDiv({ cls: 'clian-ask-question-inline' });

    const titleEl = this.rootEl.createDiv({ cls: 'clian-ask-inline-title' });
    ...
    if (!this.config.immediateSelect) {
      this.tabBar = this.rootEl.createDiv({ cls: 'clian-ask-tab-bar' });
      this.renderTabBar();
    }
    this.contentArea = this.rootEl.createDiv({ cls: 'clian-ask-content' });
  ...
  private renderTabBar(): void {
    ...
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      const tab = this.tabBar.createSpan({ cls: 'clian-ask-tab' });
      tab.createSpan({ text: this.questions[idx].header, cls: 'clian-ask-tab-label' });
      tab.createSpan({ text: answered ? ' \u2713' : '', cls: 'clian-ask-tab-tick' });
    ...
    const submitTab = this.tabBar.createSpan({ cls: 'clian-ask-tab' });
    submitTab.createSpan({ text: allAnswered ? '\u2713 ' : '', cls: 'clian-ask-tab-submit-check' });
    submitTab.createSpan({ text: 'Submit', cls: 'clian-ask-tab-label' });
  ...
  private renderQuestionTab(idx: number): void {
    ...
    this.contentArea.createDiv({
      text: q.question,
      cls: 'clian-ask-question-text',
    });

    const listEl = this.contentArea.createDiv({ cls: 'clian-ask-list' });
    ...
    for (let optIdx = 0; optIdx < q.options.length; optIdx++) {
      ...
      const row = listEl.createDiv({ cls: 'clian-ask-item' });
      if (isFocused) row.addClass('is-focused');
      if (isSelected) row.addClass('is-selected');

      row.createSpan({ text: isFocused ? '\u203A' : '\u00A0', cls: 'clian-ask-cursor' });
      row.createSpan({ text: `${optIdx + 1}. `, cls: 'clian-ask-item-num' });

      if (isMulti) {
        this.renderMultiSelectCheckbox(row, isSelected);
      }

      const labelBlock = row.createDiv({ cls: 'clian-ask-item-content' });
      const labelRow = labelBlock.createDiv({ cls: 'clian-ask-label-row' });
      labelRow.createSpan({ text: option.label, cls: 'clian-ask-item-label' });

      if (!isMulti && isSelected) {
        labelRow.createSpan({ text: ' \u2713', cls: 'clian-ask-check-mark' });
      }

      if (option.description) {
        labelBlock.createDiv({ text: option.description, cls: 'clian-ask-item-desc' });
      }
    ...
    if (this.config.showCustomInput) {
      ...
      const customRow = listEl.createDiv({ cls: 'clian-ask-item clian-ask-custom-item' });
      if (customFocused) customRow.addClass('is-focused');

      customRow.createSpan({ text: customFocused ? '\u203A' : '\u00A0', cls: 'clian-ask-cursor' });
      customRow.createSpan({ text: `${customIdx + 1}. `, cls: 'clian-ask-item-num' });
  ...
  private renderSubmitTab(): void {
    this.contentArea.createDiv({
      ...
      cls: 'clian-ask-review-title',
    });

    const reviewEl = this.contentArea.createDiv({ cls: 'clian-ask-review' });

    for (let idx = 0; idx < this.questions.length; idx++) {
      const q = this.questions[idx];
      const answerText = this.getAnswerText(idx);

      const pairEl = reviewEl.createDiv({ cls: 'clian-ask-review-pair' });
      pairEl.createDiv({ text: `${idx + 1}.`, cls: 'clian-ask-review-num' });
      const bodyEl = pairEl.createDiv({ cls: 'clian-ask-review-body' });
      bodyEl.createDiv({ text: q.question, cls: 'clian-ask-review-q-text' });
      bodyEl.createDiv({
        text: answerText || 'Not answered',
        cls: answerText ? 'clian-ask-review-a-text' : 'clian-ask-review-empty',
      });
    ...
    this.contentArea.createDiv({
      ...
      cls: 'clian-ask-review-prompt',
    });

    const actionsEl = this.contentArea.createDiv({ cls: 'clian-ask-list' });
    const allAnswered = this.questions.every((_, i) => this.isQuestionAnswered(i));

    const submitRow = actionsEl.createDiv({ cls: 'clian-ask-item' });
    if (this.focusedItemIndex === 0) submitRow.addClass('is-focused');
    if (!allAnswered) submitRow.addClass('is-disabled');
    submitRow.createSpan({ text: this.focusedItemIndex === 0 ? '\u203A' : '\u00A0', cls: 'clian-ask-cursor' });
    submitRow.createSpan({ text: '1. ', cls: 'clian-ask-item-num' });
    submitRow.createSpan({ text: 'Submit answers', cls: 'clian-ask-item-label' });
    ...
    const cancelRow = actionsEl.createDiv({ cls: 'clian-ask-item' });
    if (this.focusedItemIndex === 1) cancelRow.addClass('is-focused');
    cancelRow.createSpan({ text: this.focusedItemIndex === 1 ? '\u203A' : '\u00A0', cls: 'clian-ask-cursor' });
    cancelRow.createSpan({ text: '2. ', cls: 'clian-ask-item-num' });
    cancelRow.createSpan({ text: 'Cancel', cls: 'clian-ask-item-label' });
  ...
  private renderMultiSelectCheckbox(parent: HTMLElement, checked: boolean): void {
    parent.createSpan({
      text: checked ? '[\u2713] ' : '[ ] ',
      cls: `clian-ask-check${checked ? ' is-checked' : ''}`,
    });
  }
  ...
  private updateOptionVisuals(qIdx: number): void {
    ...
    for (let i = 0; i < q.options.length; i++) {
      ...
      if (isMulti) {
        const checkSpan = item.querySelector('.clian-ask-check') as HTMLElement | null;
        if (checkSpan) {
          checkSpan.textContent = isSelected ? '[\u2713] ' : '[ ] ';
          checkSpan.toggleClass('is-checked', isSelected);
        }
      } else {
        const labelRow = item.querySelector('.clian-ask-label-row') as HTMLElement | null;
        const existingMark = item.querySelector('.clian-ask-check-mark');
  ...
  private updateFocusIndicator(): void {
    for (let i = 0; i < this.currentItems.length; i++) {
      const item = this.currentItems[i];
      const cursor = item.querySelector('.clian-ask-cursor');
      if (i === this.focusedItemIndex) {
        item.addClass('is-focused');
        if (cursor) cursor.textContent = '\u203A';
        item.scrollIntoView({ block: 'nearest' });

        if (item.hasClass('clian-ask-custom-item')) {
          const input = item.querySelector('.clian-ask-custom-text') as HTMLInputElement;
          if (input) {
            input.focus();
            this.isInputFocused = true;
          }
  ...
  private updateTabIndicators(): void {
    for (let idx = 0; idx < this.questions.length; idx++) {
      ...
      const tick = tab.querySelector('.clian-ask-tab-tick');
      const answered = this.isQuestionAnswered(idx);
    ...
    if (submitTab) {
      const submitCheck = submitTab.querySelector('.clian-ask-tab-submit-check');
      const allAnswered = this.questions.every((_, i) => this.isQuestionAnswered(i));
