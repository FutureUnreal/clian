...
Main sidebar chat interface. `ClianView` is a thin shell; logic lives in controllers and services.

## Architecture

```
ClianView (lifecycle + assembly)
...
```
User Input → InputController → ClianService.query()
                                      ↓
...
```typescript
// ClianService created on first query, not on tab create
...
- `ClianView.onClose()` must abort all tabs and dispose services
