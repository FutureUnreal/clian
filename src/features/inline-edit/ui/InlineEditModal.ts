...
class DiffWidget extends WidgetType {
  ...
  toDOM(): HTMLElement {
    ...
    span.className = 'clian-inline-diff-replace';
    span.innerHTML = this.diffHtml;

    const btns = document.createElement('span');
    btns.className = 'clian-inline-diff-buttons';

    const rejectBtn = document.createElement('button');
    rejectBtn.className = 'clian-inline-diff-btn reject';
    rejectBtn.textContent = '✕';
    rejectBtn.title = 'Reject (Esc)';
    rejectBtn.onclick = () => this.controller.reject();

    const acceptBtn = document.createElement('button');
    acceptBtn.className = 'clian-inline-diff-btn accept';
...
function diffToHtml(ops: DiffOp[]): string {
  return ops.map(op => {
    ...
    switch (op.type) {
      case 'delete': return `<span class="clian-diff-del">${escaped}</span>`;
      case 'insert': return `<span class="clian-diff-ins">${escaped}</span>`;
...
class InlineEditController {
  ...
  private showInsertionInPlace() {
    ...
    const diffHtml = `<span class="clian-diff-ins">${escaped}</span>`;
