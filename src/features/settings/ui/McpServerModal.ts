...
export class McpServerModal extends Modal {
  ...
  private existingServer: ClianMcpServer | null;
  private onSave: (server: ClianMcpServer) => void;
  ...
  constructor(
    app: App,
    plugin: ClianPlugin,
    existingServer: ClianMcpServer | null,
    onSave: (server: ClianMcpServer) => void,
  ...
  onOpen() {
    ...
    this.modalEl.addClass('clian-mcp-modal');
    ...
    this.typeFieldsEl = contentEl.createDiv({ cls: 'clian-mcp-type-fields' });
    this.renderTypeFields();
    ...
    const buttonContainer = contentEl.createDiv({ cls: 'clian-mcp-buttons' });
  ...
  private renderStdioFields() {
    ...
    cmdSetting.settingEl.addClass('clian-mcp-cmd-setting');

    const cmdTextarea = cmdSetting.controlEl.createEl('textarea', {
      cls: 'clian-mcp-cmd-textarea',
    });
    ...
    envSetting.settingEl.addClass('clian-mcp-env-setting');

    const envTextarea = envSetting.controlEl.createEl('textarea', {
      cls: 'clian-mcp-env-textarea',
    });
  ...
  private renderUrlFields() {
    ...
    headersSetting.settingEl.addClass('clian-mcp-env-setting');
