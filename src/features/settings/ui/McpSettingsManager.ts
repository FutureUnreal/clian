...
import type { ClianMcpServer, McpServerConfig, McpServerType } from '../../../core/types';
...
export class McpSettingsManager {
  ...
  private servers: ClianMcpServer[] = [];

  /**
   * Broadcasts MCP reload to all open Clian views.
  ...
  constructor(containerEl: HTMLElement, plugin: ClianPlugin) {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'clian-mcp-header' });
    headerEl.createSpan({ text: 'MCP Servers', cls: 'clian-mcp-label' });

    const addContainer = headerEl.createDiv({ cls: 'clian-mcp-add-container' });
    ...
    const dropdown = addContainer.createDiv({ cls: 'clian-mcp-add-dropdown' });

    const stdioOption = dropdown.createDiv({ cls: 'clian-mcp-add-option' });
    setIcon(stdioOption.createSpan({ cls: 'clian-mcp-add-option-icon' }), 'terminal');
    ...
    const httpOption = dropdown.createDiv({ cls: 'clian-mcp-add-option' });
    setIcon(httpOption.createSpan({ cls: 'clian-mcp-add-option-icon' }), 'globe');
    ...
    const importOption = dropdown.createDiv({ cls: 'clian-mcp-add-option' });
    setIcon(importOption.createSpan({ cls: 'clian-mcp-add-option-icon' }), 'clipboard-paste');
    ...
    if (this.servers.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'clian-mcp-empty' });
      emptyEl.setText('No MCP servers configured. Click "Add" to add one.');
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'clian-mcp-list' });
  ...
  private renderServerItem(listEl: HTMLElement, server: ClianMcpServer) {
    const itemEl = listEl.createDiv({ cls: 'clian-mcp-item' });
    if (!server.enabled) {
      itemEl.addClass('clian-mcp-item-disabled');
    }

    const statusEl = itemEl.createDiv({ cls: 'clian-mcp-status' });
    statusEl.addClass(
      server.enabled ? 'clian-mcp-status-enabled' : 'clian-mcp-status-disabled'
    );

    const infoEl = itemEl.createDiv({ cls: 'clian-mcp-info' });

    const nameRow = infoEl.createDiv({ cls: 'clian-mcp-name-row' });

    const nameEl = nameRow.createSpan({ cls: 'clian-mcp-name' });
    nameEl.setText(server.name);

    const serverType = getMcpServerType(server.config);
    const typeEl = nameRow.createSpan({ cls: 'clian-mcp-type-badge' });
    typeEl.setText(serverType);

    if (server.contextSaving) {
      const csEl = nameRow.createSpan({ cls: 'clian-mcp-context-saving-badge' });
      csEl.setText('@');
      csEl.setAttribute('title', 'Context-saving: mention with @' + server.name + ' to enable');
    }

    const previewEl = infoEl.createDiv({ cls: 'clian-mcp-preview' });
    ...
    const actionsEl = itemEl.createDiv({ cls: 'clian-mcp-actions' });

    const testBtn = actionsEl.createEl('button', {
      cls: 'clian-mcp-action-btn',
    ...
    const deleteBtn = actionsEl.createEl('button', {
      cls: 'clian-mcp-action-btn clian-mcp-delete-btn',
  ...
  private async testServer(server: ClianMcpServer) {
  ...
  private async updateServerDisabledTools(
    ...
    try {
      ...
      await this.plugin.storage.syncMcpToCliConfigs();
    } catch (error) {
  ...
  private async updateAllDisabledTools(server: ClianMcpServer, disabledTools: string[]) {
    await this.updateServerDisabledTools(
      server,
  ...
  private getServerPreview(server: ClianMcpServer, type: McpServerType): string {
    if (type === 'stdio') {
  ...
  private openModal(existing: ClianMcpServer | null, initialType?: McpServerType) {
  ...
  private async saveServer(server: ClianMcpServer, existing: ClianMcpServer | null) {
    if (existing) {
  ...
  private async toggleServer(server: ClianMcpServer) {
  ...
  private async deleteServer(server: ClianMcpServer) {
