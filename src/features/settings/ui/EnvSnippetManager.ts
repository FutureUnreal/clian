...
export class EnvSnippetModal extends Modal {
  ...
  constructor(app: App, plugin: ClianPlugin, snippet: EnvSnippet | null, onSave: (snippet: EnvSnippet) => void) {
    super(app);
    this.plugin = plugin;
  ...
  onOpen() {
    ...
    this.modalEl.addClass('clian-env-snippet-modal');
    ...
    const renderContextLimitFields = () => {
      ...
      for (const modelId of uniqueModelIds) {
        const row = contextLimitsContainer.createDiv({ cls: 'clian-snippet-limit-row' });
        row.createSpan({ text: modelId, cls: 'clian-snippet-limit-model' });
        row.createSpan({ cls: 'clian-snippet-limit-spacer' });

        const input = row.createEl('input', {
          type: 'text',
          placeholder: '200k',
          cls: 'clian-snippet-limit-input',
        });
    ...
    envVarsSetting.settingEl.addClass('clian-env-snippet-setting');
    envVarsSetting.controlEl.addClass('clian-env-snippet-control');

    contextLimitsContainer = contentEl.createDiv({ cls: 'clian-snippet-context-limits' });
    renderContextLimitFields();

    const buttonContainer = contentEl.createDiv({ cls: 'clian-snippet-buttons' });
...
export class EnvSnippetManager {
  ...
  constructor(containerEl: HTMLElement, plugin: ClianPlugin, onContextLimitsChange?: () => void) {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'clian-snippet-header' });
    headerEl.createSpan({ text: t('settings.envSnippets.name'), cls: 'clian-snippet-label' });
    ...
    if (snippets.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'clian-snippet-empty' });
      emptyEl.setText(t('settings.envSnippets.noSnippets'));
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'clian-snippet-list' });

    for (const snippet of snippets) {
      const itemEl = listEl.createDiv({ cls: 'clian-snippet-item' });

      const infoEl = itemEl.createDiv({ cls: 'clian-snippet-info' });

      const nameEl = infoEl.createDiv({ cls: 'clian-snippet-name' });
      nameEl.setText(snippet.name);

      if (snippet.description) {
        const descEl = infoEl.createDiv({ cls: 'clian-snippet-description' });
        descEl.setText(snippet.description);
      }

      const actionsEl = itemEl.createDiv({ cls: 'clian-snippet-actions' });
  ...
  private async insertSnippet(snippet: EnvSnippet) {
    ...
    const envTextarea = document.querySelector('.clian-settings-env-textarea') as HTMLTextAreaElement;
    if (envTextarea) {
