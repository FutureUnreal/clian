...
import type { ClaudeCodePlugin } from '../../../core/types';
...
export class PluginSettingsManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'clian-plugin-header' });
    headerEl.createSpan({ text: 'Claude Code Plugins', cls: 'clian-plugin-label' });
    ...
    if (plugins.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'clian-plugin-empty' });
    ...
    const listEl = this.containerEl.createDiv({ cls: 'clian-plugin-list' });

    if (projectPlugins.length > 0) {
      const sectionHeader = listEl.createDiv({ cls: 'clian-plugin-section-header' });
  ...
  private renderPluginItem(listEl: HTMLElement, plugin: ClaudeCodePlugin) {
    const itemEl = listEl.createDiv({ cls: 'clian-plugin-item' });
    if (!plugin.enabled) {
      itemEl.addClass('clian-plugin-item-disabled');
    }

    const statusEl = itemEl.createDiv({ cls: 'clian-plugin-status' });
    if (plugin.enabled) {
      statusEl.addClass('clian-plugin-status-enabled');
    } else {
      statusEl.addClass('clian-plugin-status-disabled');
    }

    const infoEl = itemEl.createDiv({ cls: 'clian-plugin-info' });

    const nameRow = infoEl.createDiv({ cls: 'clian-plugin-name-row' });

    const nameEl = nameRow.createSpan({ cls: 'clian-plugin-name' });
    nameEl.setText(plugin.name);

    const actionsEl = itemEl.createDiv({ cls: 'clian-plugin-actions' });

    const toggleBtn = actionsEl.createEl('button', {
      cls: 'clian-plugin-action-btn',
