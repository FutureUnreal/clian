...
export type SlashCommandListKind = 'all' | SlashCommandKind;

export interface SlashCommandSettingsOptions {
  /** Which type of items to show in this list. Defaults to 'all'. */
  kind?: SlashCommandListKind;
  /** Callback invoked after save/delete/convert updates underlying storage. */
  onDidChange?: () => void;
}
...
export class SlashCommandModal extends Modal {
  ...
  private defaultType: SlashCommandKind | null;
  ...
  constructor(
    ...
    defaultType?: SlashCommandKind,
  ) {
    super(app);
    this.plugin = plugin;
    this.existingCmd = existingCmd;
    this.onSave = onSave;
    this.defaultType = defaultType ?? null;
  }

  onOpen() {
    const existingIsSkill = this.existingCmd ? isSkill(this.existingCmd) : false;
    let selectedType: SlashCommandKind = this.existingCmd
    ...
    this.modalEl.addClass('clian-sp-modal');
    ...
    // Only show the type selector when creating a new item and no default type is provided.
    if (!this.existingCmd && this.defaultType === null) {
      new Setting(contentEl)
        ...
        .addDropdown(dropdown => {
          dropdown
            ...
            .onChange(value => {
              selectedType = value as SlashCommandKind;
              this.setTitle(this.existingCmd ? `Edit ${typeLabel()}` : `Add ${typeLabel()}`);
    ...
    const details = contentEl.createEl('details', { cls: 'clian-sp-advanced-section' });
    details.createEl('summary', {
      text: 'Advanced options',
      cls: 'clian-sp-advanced-summary',
    });
    ...
    const buttonContainer = contentEl.createDiv({ cls: 'clian-sp-modal-buttons' });

    const cancelBtn = buttonContainer.createEl('button', {
      text: 'Cancel',
      cls: 'clian-cancel-btn',
    });
...
export class SlashCommandSettings {
  ...
  private plugin: ClianPlugin;
  private kind: SlashCommandListKind;
  private onDidChange?: () => void;

  constructor(containerEl: HTMLElement, plugin: ClianPlugin, options?: SlashCommandSettingsOptions) {
  ...
  private render(): void {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'clian-sp-header' });

    const headerLabel = this.kind === 'all'
      ? `${t('settings.slashCommands.name')} & ${t('settings.skills.name')}`
      : (this.kind === 'skill' ? t('settings.skills.name') : t('settings.slashCommands.name'));

    headerEl.createSpan({ text: headerLabel, cls: 'clian-sp-label' });

    const actionsEl = headerEl.createDiv({ cls: 'clian-sp-header-actions' });

    const addBtn = actionsEl.createEl('button', {
      cls: 'clian-settings-action-btn',
      attr: { 'aria-label': 'Add' },
    });
    setIcon(addBtn, 'plus');
    addBtn.addEventListener('click', () => this.openCommandModal(
      null,
      this.kind === 'all' ? undefined : this.kind
    ));

    const allCommands = this.plugin.settings.slashCommands;
    const commands = this.kind === 'skill'
      ? allCommands.filter(c => isSkill(c))
      : this.kind === 'command'
        ? allCommands.filter(c => !isSkill(c))
        : allCommands;

    if (commands.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'clian-sp-empty-state' });
      emptyEl.setText(
        this.kind === 'skill'
          ? 'No skills configured. Click + to create one.'
          : this.kind === 'command'
            ? 'No slash commands configured. Click + to create one.'
            : 'No commands or skills configured. Click + to create one.'
      );
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'clian-sp-list' });
  ...
  private renderCommandItem(listEl: HTMLElement, cmd: SlashCommand): void {
    const itemEl = listEl.createDiv({ cls: 'clian-sp-item' });

    const infoEl = itemEl.createDiv({ cls: 'clian-sp-info' });

    const headerRow = infoEl.createDiv({ cls: 'clian-sp-item-header' });

    const nameEl = headerRow.createSpan({ cls: 'clian-sp-item-name' });
    nameEl.setText(`/${cmd.name}`);

    if (this.kind === 'all' && isSkill(cmd)) {
      headerRow.createSpan({ text: 'skill', cls: 'clian-slash-item-badge' });
    }

    if (cmd.argumentHint) {
      const hintEl = headerRow.createSpan({ cls: 'clian-slash-item-hint' });
      hintEl.setText(cmd.argumentHint);
    }

    if (cmd.description) {
      const descEl = infoEl.createDiv({ cls: 'clian-sp-item-desc' });
      descEl.setText(cmd.description);
    }

    const actionsEl = itemEl.createDiv({ cls: 'clian-sp-item-actions' });
    ...
    const deleteBtn = actionsEl.createEl('button', {
      cls: 'clian-settings-action-btn clian-settings-delete-btn',
  ...
  private openCommandModal(existingCmd: SlashCommand | null, defaultType?: SlashCommandKind): void {
  ...
  private async saveCommand(cmd: SlashCommand, existing: SlashCommand | null): Promise<void> {
    ...
    await this.plugin.storage.syncCommandsAndSkillsToCliDirs();
