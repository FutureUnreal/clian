...
function formatToggleError(error: unknown): string {
  ...
  if (msg.includes('permission') || msg.includes('eacces')) {
    return 'Permission denied. Check .clian/ folder permissions.';
  }
  ...
  if (msg.includes('json') || msg.includes('syntax')) {
    return 'Config file corrupted. Check .clian/mcp.json';
  }
...
export class McpTestModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('clian-mcp-test-modal');
  ...
  private renderLoading() {
    ...
    const loadingEl = this.contentEl_.createDiv({ cls: 'clian-mcp-test-loading' });

    const spinnerEl = loadingEl.createDiv({ cls: 'clian-mcp-test-spinner' });
  ...
  private render() {
    ...
    const statusEl = this.contentEl_.createDiv({ cls: 'clian-mcp-test-status' });

    const iconEl = statusEl.createSpan({ cls: 'clian-mcp-test-icon' });
    ...
    const textEl = statusEl.createSpan({ cls: 'clian-mcp-test-text' });
    if (this.result.success) {
    ...
    if (this.result.error) {
      const errorEl = this.contentEl_.createDiv({ cls: 'clian-mcp-test-error' });
    ...
    if (this.result.tools.length > 0) {
      const toolsSection = this.contentEl_.createDiv({ cls: 'clian-mcp-test-tools' });

      const toolsHeader = toolsSection.createDiv({ cls: 'clian-mcp-test-tools-header' });
      toolsHeader.setText(`Available Tools (${this.result.tools.length})`);

      const toolsList = toolsSection.createDiv({ cls: 'clian-mcp-test-tools-list' });

      for (const tool of this.result.tools) {
        this.renderTool(toolsList, tool);
      }
    } else if (this.result.success) {
      const noToolsEl = this.contentEl_.createDiv({ cls: 'clian-mcp-test-no-tools' });
      noToolsEl.setText('No tools information available. Tools will be loaded when used in chat.');
    }

    const buttonContainer = this.contentEl_.createDiv({ cls: 'clian-mcp-test-buttons' });

    if (this.result.tools.length > 0 && this.onToolToggle) {
      this.toggleAllBtn = buttonContainer.createEl('button', {
        cls: 'clian-mcp-toggle-all-btn',
      });
  ...
  private renderTool(container: HTMLElement, tool: McpTool) {
    const toolEl = container.createDiv({ cls: 'clian-mcp-test-tool' });

    const headerEl = toolEl.createDiv({ cls: 'clian-mcp-test-tool-header' });

    const iconEl = headerEl.createSpan({ cls: 'clian-mcp-test-tool-icon' });
    setIcon(iconEl, 'wrench');

    const nameEl = headerEl.createSpan({ cls: 'clian-mcp-test-tool-name' });
    nameEl.setText(tool.name);

    const toggleEl = headerEl.createDiv({ cls: 'clian-mcp-test-tool-toggle' });
    ...
    if (tool.description) {
      const descEl = toolEl.createDiv({ cls: 'clian-mcp-test-tool-desc' });
  ...
  private updateToolState(toolEl: HTMLElement, enabled: boolean) {
    toolEl.toggleClass('clian-mcp-test-tool-disabled', !enabled);
  }
