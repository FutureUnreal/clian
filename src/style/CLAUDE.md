...
├── settings/       # base (shared .clian-sp-* panel layout), env-snippets, slash-settings, mcp-settings, plugin-settings, agent-settings
├── accessibility.css
...
- **Prefix**: All classes use `.clian-` prefix
- **BEM-lite**: `.clian-{block}`, `.clian-{block}-{element}`, `.clian-{block}--{modifier}`
