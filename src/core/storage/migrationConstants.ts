 ...
 * from settings.json to `.clian/settings.json`.
 */

/**
 * Fields that are Clian-specific and should NOT be in CC settings.json.
 * These are migrated to `.clian/settings.json` and stripped from settings.json.
 *
 * IMPORTANT: Keep this list updated when adding new Clian settings!
 */
export const CLIAN_ONLY_FIELDS = new Set([
  // User preferences
  'userName',
  ...
  // Environment (Clian uses string format + snippets)
...
/**
 * Fields that are Clian-specific and should be migrated.
 * Excludes deprecated fields which are just removed.
 */
export const MIGRATABLE_CLIAN_FIELDS = new Set([
  'userName',
  'enableBlocklist',
  'blockedCommands',
  'permissionMode',
...
/**
 * Convert CC env object format to Clian environmentVariables string format.
 *
 * @example
