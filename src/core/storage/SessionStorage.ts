...
export class SessionStorage {
  constructor(
    ...
    private sessionsPath: string = SESSIONS_PATH,
  ) { }
  ...
  async listConversations(): Promise<ConversationMeta[]> {
    ...
    try {
      const files = await this.adapter.listFiles(this.sessionsPath);
  ...
  getFilePath(id: string): string {
    return `${this.sessionsPath}/${id}.jsonl`;
  }
  ...
  async isNativeSession(id: string): Promise<boolean> {
    const legacyPath = `${this.sessionsPath}/${id}.jsonl`;
  ...
  async listNativeMetadata(): Promise<SessionMetadata[]> {
    ...
    try {
      ...
      for (const filePath of metaFiles) {
        // Extract ID from path: {sessionsPath}/{id}.meta.json
