 ...
 *
 * Plugin-specific settings go in `.clian/settings.json`.
 */
...
import { CLIAN_ONLY_FIELDS } from './migrationConstants';
...
function hasClianOnlyFields(data: Record<string, unknown>): boolean {
  return Object.keys(data).some(key => CLIAN_ONLY_FIELDS.has(key));
}

/**
 * Check if a settings object uses the legacy permissions format.
...
export class CCSettingsStorage {
   ...
   *
   * @param stripClianFields - If true, remove Clian-only fields (only during migration)
   */
  async save(settings: CCSettings, stripClianFields: boolean = false): Promise<void> {
    ...
    if (await this.adapter.exists(CC_SETTINGS_PATH)) {
      try {
        ...
        // Only strip Clian-only fields during explicit migration
        if (stripClianFields && (isLegacyPermissionsFormat(parsed) || hasClianOnlyFields(parsed))) {
          existing = {};
          for (const [key, value] of Object.entries(parsed)) {
            if (!CLIAN_ONLY_FIELDS.has(key)) {
              existing[key] = value;
            }
          }
