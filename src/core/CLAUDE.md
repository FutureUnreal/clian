...
|--------|---------|-----------|
| `agent/` | Claude Agent SDK wrapper | `ClianService` (incl. fork session tracking), `SessionManager`, `QueryOptionsBuilder` (incl. `resumeSessionAt`), `MessageChannel`, `customSpawn` |
...
| `storage/` | Persistence layer | `StorageService`, `SessionStorage`, `CCSettingsStorage`, `ClianSettingsStorage`, `McpStorage`, `SkillStorage`, `SlashCommandStorage`, `VaultFileAdapter` |
...
const service = new ClianService(plugin, vaultPath);
...
- `ClianService` must be disposed on tab close (abort + cleanup)
- `SessionManager` handles SDK session resume via `sessionId`
- Fork uses `pendingForkSession` + `pendingResumeAt` on `ClianService` to pass `resumeSessionAt` to SDK; these are one-shot flags consumed on the next query
