/**
 * Clian - MCP (Model Context Protocol) type definitions
 *
...
/** Extended server configuration with Clian-specific options. */
export interface ClianMcpServer {
...
/** Extended config file with Clian metadata. */
export interface ClianMcpConfigFile extends McpConfigFile {
  _clian?: {
    /** Per-server Clian-specific settings. */
    servers: Record<
      string,
      {
        enabled?: boolean;
...
export const DEFAULT_MCP_SERVER: Omit<ClianMcpServer, 'name' | 'config'> = {
  enabled: true,
  contextSaving: true,
};
