...
import type { CodexReasoningEffort, GeminiThinkingMode } from './engines';
...
/** Which backend to use for auto title generation. */
export type TitleGenerationEngine = 'auto' | 'claude' | 'codex' | 'gemini';
...
/**
 * Plugin settings stored in `.clian/settings.json`.
 * These settings are NOT shared with any CLI.
 */
export interface ClianSettings {
  // User preferences
  userName: string;

  // Security (Clian-specific, CC uses permissions.deny instead)
  enableBlocklist: boolean;
  blockedCommands: PlatformBlockedCommands;
  permissionMode: PermissionMode;

  // Model & thinking (Clian uses enum, CC uses full model ID string)
  model: ClaudeModel;
  thinkingBudget: ThinkingBudget;
  /** Extra Claude model IDs to show in the model picker. */
  extraClaudeModels: ClaudeModel[];
  /** Default Codex model override (empty = Codex default). */
  codexModel: string;
  /** Codex reasoning effort override. */
  codexReasoningEffort: CodexReasoningEffort;
  /** Default Gemini model override (empty = Gemini default). */
  geminiModel: string;
  /** Gemini thinking mode override. */
  ...
  /** Per-device Codex command overrides keyed by hostname (empty = auto-detect). */
  codexCliCommandsByHost: HostnameCliPaths;
  /** Per-device Gemini command overrides keyed by hostname (empty = auto-detect). */
  geminiCliCommandsByHost: HostnameCliPaths;
...
/** Default Clian settings. */
export const DEFAULT_SETTINGS: ClianSettings = {
  // User preferences
  userName: '',
