/**
 * Clian - Claude Agent SDK wrapper
 *
...
import { TOOL_AGENT_OUTPUT, TOOL_ASK_USER_QUESTION, TOOL_ENTER_PLAN_MODE, TOOL_EXIT_PLAN_MODE, TOOL_SKILL } from '../tools/toolNames';
import type {
  ApprovalDecision,
  ChatMessage,
  Conversation,
...
export class ClianService {
  ...
  private streamedBlockTextByHandler = new Map<string, Map<string, string>>();
  ...
  private static readonly TURN_STDERR_MAX_CHARS = 8000;
  private turnStderrText = '';

  constructor(plugin: ClianPlugin, mcpManager: McpServerManager) {
    this.plugin = plugin;
    this.mcpManager = mcpManager;
  }

  private resetTurnStderr(): void {
    this.turnStderrText = '';
  }

  private appendTurnStderr(data: string): void {
    if (!data) return;

    this.turnStderrText += String(data);
    if (this.turnStderrText.length > ClianService.TURN_STDERR_MAX_CHARS) {
      this.turnStderrText = this.turnStderrText.slice(-ClianService.TURN_STDERR_MAX_CHARS);
    }
  }
  ...
  private static redactSecrets(text: string): string {
    return text
      ...
      // Env-style secrets
      .replace(
        /\b(ANTHROPIC_API_KEY|OPENAI_API_KEY|GEMINI_API_KEY|CLAUDE_CODE_OAUTH_TOKEN)\b\s*[:=]\s*([^\s]+)/gi,
        '$1=***'
      );
  }

  private getTurnStderrSnippet(): string {
    const trimmed = this.turnStderrText.trim();
    if (!trimmed) return '';
    const cleaned = ClianService.stripAnsi(trimmed);
    return ClianService.redactSecrets(cleaned);
  }

  private static buildStreamedBlockKey(
  ...
  private static computePrefixDelta(previous: string, next: string): string {
    if (!next) return '';
  ...
  private async routeMessage(message: SDKMessage): Promise<void> {
    ...
    const handler = this.responseHandlers[this.responseHandlers.length - 1];
    const streamedByKey = handler
      ? (this.streamedBlockTextByHandler.get(handler.id) ?? new Map<string, string>())
      : null;
    if (handler && !this.streamedBlockTextByHandler.has(handler.id)) {
      this.streamedBlockTextByHandler.set(handler.id, streamedByKey!);
    }

    // Assistant messages include a terminal snapshot of content blocks. When streaming is enabled,
    // some SDK/CLI versions also emit deltas via stream_event. Track contentBlockIndex so we can
    // emit only the missing delta (if any) instead of dropping or duplicating content.
    let assistantContentBlockIndex = 0;
    ...
    for (const event of transformSDKMessage(message, this.getTransformOptions())) {
      if (isSessionInitEvent(event)) {
        ...
        let chunkToEmit: StreamChunk | null = event;

        if (handler && streamedByKey) {
          if (message.type === 'stream_event' && (event.type === 'text' || event.type === 'thinking')) {
            const streamIndex = typeof message.event?.index === 'number' ? message.event.index : 0;
            const key = ClianService.buildStreamedBlockKey(event.parentToolUseId, streamIndex, event.type);
            const prev = streamedByKey.get(key) ?? '';
            streamedByKey.set(key, prev + event.content);
            handler.markStreamTextSeen();
          }

          if (message.type === 'assistant') {
            if (event.type === 'text' || event.type === 'thinking') {
              const key = ClianService.buildStreamedBlockKey(event.parentToolUseId, assistantContentBlockIndex, event.type);
              const prev = streamedByKey.get(key) ?? '';
              const delta = ClianService.computePrefixDelta(prev, event.content);
              streamedByKey.set(key, event.content);
              assistantContentBlockIndex++;
              if (!delta) {
                chunkToEmit = null;
              } else {
                chunkToEmit = { ...event, content: delta } as StreamChunk;
              }
        ...
        if (handler && chunkToEmit) {
          // Add sessionId to usage chunks (consistent with cold-start path)
          if (chunkToEmit.type === 'usage') {
            handler.onChunk({ ...chunkToEmit, sessionId: this.sessionManager.getSessionId() });
          } else {
            handler.onChunk(chunkToEmit);
          }
        }
      }
    }
  ...
  private registerResponseHandler(handler: ResponseHandler): void {
    ...
    this.streamedBlockTextByHandler.set(handler.id, new Map());
  }
  ...
  private unregisterResponseHandler(handlerId: string): void {
    ...
    this.streamedBlockTextByHandler.delete(handlerId);
  }
  ...
  private async *queryViaPersistent(
    ...
    const state = {
      ...
      sawRenderableChunk: false,
    };

    const hiddenToolIds = new Set<string>();

    const isRenderableChunk = (chunk: StreamChunk): boolean => {
      switch (chunk.type) {
        case 'text':
        case 'thinking':
          return typeof chunk.content === 'string' && chunk.content.trim().length > 0;
        case 'tool_use':
          if (chunk.name === TOOL_AGENT_OUTPUT) {
            hiddenToolIds.add(chunk.id);
            return false;
          }
          return true;
        case 'tool_result':
          if (hiddenToolIds.has(chunk.id)) {
            return typeof chunk.content === 'string' && chunk.content.trim().length > 0;
          }
          return true;
        case 'blocked':
    ...
    const handler = createResponseHandler({
      ...
      onChunk: (chunk) => {
        ...
        if (!state.sawRenderableChunk && isRenderableChunk(chunk)) {
          state.sawRenderableChunk = true;
        }
    ...
    try {
      ...
      // Defensive: Claude Code can occasionally return an empty successful result (no text/tool blocks).
      // Surface a helpful message instead of leaving an empty bubble with only the duration footer.
      if (!state.sawRenderableChunk && !state.error) {
        const stderrSnippet = this.getTurnStderrSnippet();
        yield {
          type: 'error',
          content:
            `Claude returned an empty response for model "${selectedModel}".\n\n` +
            (this.plugin.settings.loadUserClaudeSettings
              ? ''
              : 'Tip: Enable "Load ~/.claude/settings.json" so Claude Code can load your auth/proxy settings (e.g. cc-switch).\n') +
            'If it still happens, check auth (ANTHROPIC_API_KEY / CLAUDE_CODE_OAUTH_TOKEN) and try switching models (e.g. "claude-sonnet-4-6" or "claude-haiku-4-5").' +
            (stderrSnippet ? `\n\nDetails (stderr):\n${stderrSnippet}` : ''),
        };
      }
  ...
  private isStreamTextEvent(message: SDKMessage): boolean {
    ...
    if (event.type === 'content_block_start') {
      // Only consider it "streamed text" if there is actual text content.
      // Some SDK versions emit a text block start event with empty payload and
      // deliver the final text only in the terminal assistant message.
      return (
        event.content_block?.type === 'text' &&
        typeof (event.content_block as { text?: unknown }).text === 'string' &&
        ((event.content_block as { text: string }).text).length > 0
      );
    }
    if (event.type === 'content_block_delta') {
      return (
        ...
        ((event.delta as { text: string }).text).length > 0
      );
    }
    return false;
  }
      ...
      for await (const message of response) {
        ...
        for (const event of transformSDKMessage(message, this.getTransformOptions(selectedModel))) {
          if (isSessionInitEvent(event)) {
            ...
            if (chunkToEmit && !sawRenderableChunk && isRenderableChunk(chunkToEmit)) {
              sawRenderableChunk = true;
            }
            if (chunkToEmit) {
              if (chunkToEmit.type === 'usage') {
                yield { ...chunkToEmit, sessionId: streamSessionId };
              } else {
                yield chunkToEmit;
              }
            }
  ...
  private async createRewindBackup(filesChanged: string[] | undefined): Promise<{
    ...
    const backupRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'clian-rewind-'));
