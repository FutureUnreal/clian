 ...
 *
 * Extracts options-building logic from ClianService for:
...
import type { ClianSettings, PermissionMode } from '../types';
 ...
 * Passed to builder methods to avoid direct dependencies on ClianService.
 */
export interface QueryOptionsContext {
  ...
  /** Optional stderr sink for Claude Code process output. */
...
export class QueryOptionsBuilder {
  ...
  static needsRestart(
    ...
    // Note: Permission mode is handled dynamically via setPermissionMode() in ClianService.
  ...
  static buildPersistentQueryOptions(ctx: PersistentQueryContext): Options {
    ...
    const options: Options = {
      ...
      env: {
        ...
        ...(process.platform === 'win32' ? { Path: ctx.enhancedPath } : {}),
      },
    ...
    options.spawnClaudeCodeProcess = createCustomSpawnFunction(ctx.enhancedPath, ctx.onStderr);
  ...
  static buildColdStartQueryOptions(ctx: ColdStartQueryContext): Options {
    ...
    const options: Options = {
      ...
      abortController: ctx.abortController,
      pathToClaudeCodeExecutable: ctx.cliPath,
      // User settings may contain permission rules that bypass Clian's permission system
      settingSources: ctx.settings.loadUserClaudeSettings
        ? ['user', 'project']
        : ['project'],
      env: {
  ...
  private static applyExtraArgs(options: Options, settings: ClianSettings): void {
