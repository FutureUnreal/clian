/**
 * Clian - Obsidian plugin entry point
 *
...
import { ClianView } from './features/chat/ClianView';
import { type InlineEditContext, InlineEditModal } from './features/inline-edit/ui/InlineEditModal';
import { ClianSettingTab } from './features/settings/ClianSettings';
...
/**
 * Main plugin class for Clian.
 * Handles plugin lifecycle, settings persistence, and conversation management.
 */
export default class ClianPlugin extends Plugin {
  settings: ClianSettings;
  ...
  async onload() {
    ...
    this.registerView(VIEW_TYPE_CLIAN, (leaf) => new ClianView(leaf, this, VIEW_TYPE_CLIAN));
    ...
    this.addCommand({
      ...
      checkCallback: (checking: boolean) => {
        const leaf = this.getChatLeaves()[0];
        if (!leaf) return false;

        const view = leaf.view as ClianView;
    ...
    this.addCommand({
      ...
      checkCallback: (checking: boolean) => {
        ...
        if (!checking) {
          tabManager.createTab(null, undefined, 'codex');
        }
        return true;
      },
    });
    ...
    this.addCommand({
      ...
      checkCallback: (checking: boolean) => {
        ...
        if (!checking) {
          tabManager.createTab(null, undefined, 'gemini');
        }
        return true;
      },
    });
    ...
    this.addSettingTab(new ClianSettingTab(this.app, this));
  }
  ...
  async loadSettings() {
    ...
    const { clian } = await this.storage.initialize();
    ...
    for (const conversation of legacyConversations) {
      ...
      conversation.flavor = meta.flavor ?? conversation.flavor;
    ...
    // Load non-Claude conversations stored under the shared layer (.clian/sessions)
    const { conversations: sharedConversations, failedCount: sharedFailedCount } =
      await this.storage.sharedSessions.loadAllConversations();

    this.conversations = [...legacyConversations, ...nativeConversations, ...sharedConversations].sort(
      (a, b) => (b.lastResponseAt ?? b.updatedAt) - (a.lastResponseAt ?? a.updatedAt)
    );

    const totalFailed = failedCount + sharedFailedCount;
    if (totalFailed > 0) {
      new Notice(`Failed to load ${totalFailed} conversation${totalFailed > 1 ? 's' : ''}`);
    }
    ...
    for (const conv of conversationsToSave) {
      if (conv.isNative) {
        ...
        // Legacy session: save full JSONL (route by flavor)
        const flavor = conv.flavor ?? 'claude';
        if (flavor === 'claude') {
          await this.storage.sessions.saveConversation(conv);
        } else {
          await this.storage.sharedSessions.saveConversation(conv);
        }
      }
    }
  }
  ...
  async saveSettings() {
    ...
    await this.storage.saveClianSettings(settingsToSave);
  }
      ...
      if (changed) {
        for (const tab of tabManager.getAllTabs()) {
          ...
          try {
            ...
            if (tab.service.flavor === 'claude' && 'resetSession' in tab.service) {
              (tab.service as unknown as { resetSession: () => void }).resetSession();
   ...
   */
  async createConversation(sessionId?: string, flavor: ChatFlavor = 'claude'): Promise<Conversation> {
    if (flavor !== 'claude') {
      ...
      await this.storage.sharedSessions.saveConversation(conversation);
      return conversation;
    }
  ...
  async deleteConversation(id: string): Promise<void> {
    ...
    const flavor = conversation.flavor ?? 'claude';
    if (flavor === 'claude') {
      const vaultPath = getVaultPath(this.app);
    ...
    if (conversation.isNative) {
      ...
      // Legacy session: delete JSONL file (route by flavor)
      if (flavor === 'claude') {
        await this.storage.sessions.deleteConversation(id);
      } else {
        await this.storage.sharedSessions.deleteConversation(id);
      }
    }
  ...
  /** Returns the active Clian view from workspace, if open. */
  getView(): ClianView | null {
    const leaves = this.getChatLeaves();
    if (leaves.length > 0) {
      return leaves[0].view as ClianView;
    }
    return null;
  }

  /** Returns all open Clian views in the workspace. */
  getAllViews(): ClianView[] {
    const leaves = this.getChatLeaves();
    return leaves.map(leaf => leaf.view as ClianView);
  }

  /**
   * Checks if a conversation is open in any Clian view.
   * Returns the view and tab if found, null otherwise.
   */
  findConversationAcrossViews(conversationId: string): { view: ClianView; tabId: string } | null {
    for (const view of this.getAllViews()) {
      const tabManager = view.getTabManager();
      if (!tabManager) continue;
