...
function createWikilink(
  ...
  link.className = 'clian-file-link internal-link';
 ...
 * Handles both our custom .clian-file-link and Obsidian's .internal-link.
 */
export function registerFileLinkHandler(
  ...
  component.registerDomEvent(container, 'click', (event: MouseEvent) => {
    ...
    const link = target.closest('.clian-file-link, .internal-link') as HTMLAnchorElement;
  ...
  const walker = document.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode(node) {
        ...
        if (parent.closest('pre, code, a, .clian-file-link, .internal-link')) {
          return NodeFilter.FILTER_REJECT;
        }
