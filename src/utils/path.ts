...
export function expandHomePath(p: string): string {
  ...
  const ppath = process.platform === 'win32' ? path.win32 : path.posix;
  if (expanded === '~') {
    return os.homedir();
  }
  if (expanded.startsWith('~/')) {
    return ppath.join(os.homedir(), expanded.slice(2));
  }
  if (expanded.startsWith('~\\')) {
    return ppath.join(os.homedir(), expanded.slice(2));
  }
...
function getPlatformPath(isWindows: boolean): PlatformPath {
  return isWindows ? path.win32 : path.posix;
}

function findFirstExistingPath(ppath: PlatformPath, entries: string[], candidates: string[]): string | null {
  for (const dir of entries) {
    if (!dir) continue;
    for (const candidate of candidates) {
      const fullPath = ppath.join(dir, candidate);
...
function resolveCliJsNearPathEntry(ppath: PlatformPath, entry: string, isWindows: boolean): string | null {
  const directCandidate = ppath.join(entry, 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js');
  if (isExistingFile(directCandidate)) {
    return directCandidate;
  }

  const baseName = ppath.basename(entry).toLowerCase();
  if (baseName === 'bin') {
    const prefix = ppath.dirname(entry);
    const candidate = isWindows
      ? ppath.join(prefix, 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js')
      : ppath.join(prefix, 'lib', 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js');
...
function resolveCliJsFromPathEntries(ppath: PlatformPath, entries: string[], isWindows: boolean): string | null {
  for (const entry of entries) {
    const candidate = resolveCliJsNearPathEntry(ppath, entry, isWindows);
    if (candidate) {
      return candidate;
    }
  }
...
function resolveClaudeFromPathEntries(
  ...
  if (!isWindows) {
    const unixCandidate = findFirstExistingPath(ppath, entries, ['claude']);
    return unixCandidate;
  }

  const exeCandidate = findFirstExistingPath(ppath, entries, ['claude.exe', 'claude']);
  if (exeCandidate) {
    return exeCandidate;
  }

  const cliJsCandidate = resolveCliJsFromPathEntries(ppath, entries, isWindows);
  if (cliJsCandidate) {
...
function getNpmGlobalPrefix(): string | null {
  ...
  if (process.platform === 'win32') {
    const ppath = path.win32;
    const appDataNpm = process.env.APPDATA
      ? ppath.join(process.env.APPDATA, 'npm')
      : null;
...
function getNpmCliJsPaths(): string[] {
  ...
  const isWindows = process.platform === 'win32';
  const ppath = getPlatformPath(isWindows);
  const cliJsPaths: string[] = [];

  if (isWindows) {
    cliJsPaths.push(
      ppath.join(homeDir, 'AppData', 'Roaming', 'npm', 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js')
    );

    const npmPrefix = getNpmGlobalPrefix();
    if (npmPrefix) {
      cliJsPaths.push(
        ppath.join(npmPrefix, 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js')
      );
    }
    ...
    cliJsPaths.push(
      ppath.join(programFiles, 'nodejs', 'node_global', 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js'),
      ppath.join(programFilesX86, 'nodejs', 'node_global', 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js')
    );

    cliJsPaths.push(
      ppath.join('D:', 'Program Files', 'nodejs', 'node_global', 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js')
    );
  } else {
    cliJsPaths.push(
      ppath.join(homeDir, '.npm-global', 'lib', 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js'),
    ...
    if (process.env.npm_config_prefix) {
      cliJsPaths.push(
        ppath.join(process.env.npm_config_prefix, 'lib', 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js')
      );
    }
  }
...
export function findClaudeCLIPath(pathValue?: string): string | null {
  ...
  if (customEntries.length > 0) {
    const customResolution = resolveClaudeFromPathEntries(ppath, customEntries, isWindows);
    if (customResolution) {
  ...
  if (isWindows) {
    const exePaths: string[] = [
      ppath.join(homeDir, '.claude', 'local', 'claude.exe'),
      ppath.join(homeDir, 'AppData', 'Local', 'Claude', 'claude.exe'),
      ppath.join(process.env.ProgramFiles || 'C:\\Program Files', 'Claude', 'claude.exe'),
      ppath.join(process.env['ProgramFiles(x86)'] || 'C:\\Program Files (x86)', 'Claude', 'claude.exe'),
      ppath.join(homeDir, '.local', 'bin', 'claude.exe'),
    ];
  ...
  const commonPaths: string[] = [
    ppath.join(homeDir, '.claude', 'local', 'claude'),
    ppath.join(homeDir, '.local', 'bin', 'claude'),
    ppath.join(homeDir, '.volta', 'bin', 'claude'),
    ppath.join(homeDir, '.asdf', 'shims', 'claude'),
    ppath.join(homeDir, '.asdf', 'bin', 'claude'),
    '/usr/local/bin/claude',
    '/opt/homebrew/bin/claude',
    ppath.join(homeDir, 'bin', 'claude'),
    ppath.join(homeDir, '.npm-global', 'bin', 'claude'),
  ];

  const npmPrefix = getNpmGlobalPrefix();
  if (npmPrefix) {
    commonPaths.push(ppath.join(npmPrefix, 'bin', 'claude'));
  }
  ...
  if (envEntries.length > 0) {
    const envResolution = resolveClaudeFromPathEntries(ppath, envEntries, isWindows);
    if (envResolution) {
...
export function translateMsysPath(value: string): string {
  ...
  if (msysMatch) {
    ...
    // MSYS root drive form: `/c` should map to `C:\` (not `C:`).
    // `C:` is drive-relative and `path.win32.normalize('C:')` becomes `C:.`,
    // which breaks prefix comparisons (and is not what users mean).
    const normalizedRest = restOfPath ? restOfPath.replace(/\//g, '\\') : '\\';

    // Convert forward slashes to backslashes for the rest of the path
    return `${driveLetter}:${normalizedRest}`;
  }
...
export function getPathAccessType(
  ...
  if (resolvedCandidate === claudeDir || resolvedCandidate.startsWith(claudeDir + '/')) {
    ...
    const safeFiles = ['mcp.json', 'settings.json', 'settings.local.json'];
