...
function getExtraBinaryPaths(): string[] {
  ...
  if (isWindows) {
    const paths: string[] = [];
    // Obsidian (Electron) may not inherit a full environment from the user's shell.
    // Fall back to conventional locations derived from the home directory.
    const localAppData = process.env.LOCALAPPDATA || (home ? path.join(home, 'AppData', 'Local') : undefined);
    const appData = process.env.APPDATA || (home ? path.join(home, 'AppData', 'Roaming') : undefined);
    ...
    // pnpm: common global bin location on Windows (pnpm setup sets PNPM_HOME)
    const pnpmHome = process.env.PNPM_HOME || (localAppData ? path.join(localAppData, 'pnpm') : undefined);
    if (pnpmHome) {
      paths.push(pnpmHome);
    }

    // Yarn: common global bin location on Windows
    if (localAppData) {
