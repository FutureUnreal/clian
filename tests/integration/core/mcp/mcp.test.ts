...
describe('McpStorage', () => {
  ...
  describe('load/save', () => {
    it('should save MCP config with _clian metadata', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
        _clian: { extra: { keep: true } },
        other: { keep: true },
      };
      ...
      expect(saved.other).toBeUndefined();
    ...
    it('should keep existing _clian fields when metadata is defaulted', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
      ...
      const saved = JSON.parse(files.get(MCP_CONFIG_PATH) || '{}') as Record<string, unknown>;
      expect(saved._clian).toEqual({ extra: { keep: true } });
    ...
    it('should remove _clian when only servers metadata exists', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
...
describe('McpServerManager', () => {
  function createManager(servers: ClianMcpServer[]): McpServerManager {
  ...
  describe('getActiveServers', () => {
    ...
    it('should return empty object for all disabled servers', () => {
      const disabledServers: ClianMcpServer[] = [
