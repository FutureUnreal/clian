...
import { DEFAULT_SETTINGS, VIEW_TYPE_CLIAN } from '@/core/types';
import * as sdkSession from '@/utils/sdkSession';

// Mock fs for ClianService
jest.mock('fs');

// Now import the plugin after mocking
import ClianPlugin from '@/main';

describe('ClianPlugin', () => {
  let plugin: ClianPlugin;
  let mockApp: any;
  ...
  beforeEach(() => {
    ...
    plugin = new ClianPlugin(mockApp, mockManifest);
  ...
  describe('onload', () => {
    ...
    it('should register the view', async () => {
      ...
      expect((plugin.registerView as jest.Mock)).toHaveBeenCalledWith(VIEW_TYPE_CLIAN, expect.any(Function));
    });
    ...
    it('should migrate legacy cli path to hostname-based paths and clear old field', async () => {
      ...
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        // claudeCliPath is stored in plugin settings
        return path === '.clian/settings.json';
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
        if (path === '.clian/settings.json') {
  ...
  describe('onunload', () => {
    // Note: With multi-tab, cleanup is handled per-tab via ClianView.onClose()
  ...
  describe('loadSettings', () => {
    it('should merge saved data with defaults', async () => {
      // Mock plugin settings exists with custom values
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        return path === '.clian/settings.json';
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
    ...
    it('should normalize blockedCommands when stored value is partial', async () => {
      // Mock plugin settings exists with partial blockedCommands
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        return path === '.clian/settings.json';
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
    ...
    it('should reconcile model from environment and persist when changed', async () => {
      // Mock plugin settings with environment variables
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        return path === '.clian/settings.json';
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
  ...
  describe('saveSettings', () => {
    it('should save settings to file', async () => {
      ...
      // Plugin settings should be written to .clian/settings.json
      ...
      // Permissions are now in .claude/settings.json (CC format), not .clian/settings.json
  ...
  describe('loadSettings with conversations', () => {
    ...
    it('should clear session IDs when provider base URL changes', async () => {
      ...
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
        if (path === '.clian/settings.json') {
          // All these fields are now in .clian/settings.json
          return JSON.stringify({
