...
describe('parsePathEntries', () => {
  ...
  it('splits on platform separator', () => {
    const sep = isWindows ? ';' : ':';
    const entries = isWindows ? ['C:\\a', 'C:\\b', 'C:\\c'] : ['/a', '/b', '/c'];
    const result = parsePathEntries(entries.join(sep));
...
describe('findClaudeCLIPath', () => {
  ...
  it('falls back to PATH environment when common and npm paths fail', () => {
    ...
    const envClaudePath = isWindows ? path.join(envDir, 'claude.exe') : path.join(envDir, 'claude');
    const originalPath = process.env.PATH;
    const sep = isWindows ? ';' : ':';
    process.env.PATH = `${envDir}${sep}${originalPath}`;
