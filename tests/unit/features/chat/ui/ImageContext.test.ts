...
function createContainerWithInputWrapper(): { container: any; inputWrapper: any } {
  ...
  const inputWrapper = container.createDiv({ cls: 'clian-input-wrapper' });
...
describe('ImageContextManager', () => {
  ...
  describe('constructor with previewContainerEl', () => {
    it('should use previewContainerEl when provided', () => {
      ...
      const previewEl = previewContainer.querySelector('.clian-image-preview');
      expect(previewEl).not.toBeNull();
    });

    it('should insert image preview before file indicator if present', () => {
      const previewContainer = createMockEl();
      const fileIndicator = previewContainer.createDiv({ cls: 'clian-file-indicator' });
      ...
      const previewIdx = children.findIndex((el: any) => el.hasClass?.('clian-image-preview'));
...
describe('ImageContextManager - Private Helpers', () => {
  ...
  describe('Drag and Drop handlers', () => {
    ...
    it('handleDrop should delegate non-image files to file drop callback', async () => {
      const addImageSpy = jest.spyOn(manager as any, 'addImageFromFile').mockResolvedValue(true);
      ...
      expect(manager['callbacks'].onFilesDropped).toHaveBeenCalledWith([mockFile], event);
      addImageSpy.mockRestore();
    });

    it('handleDrop should delegate empty file lists for text-based drops', async () => {
      const event = {
        preventDefault: jest.fn(),
        stopPropagation: jest.fn(),
      ...
      expect(manager['callbacks'].onFilesDropped).toHaveBeenCalledWith([], event);
    });
  ...
  describe('Image preview rendering', () => {
    ...
    it('renderImagePreview should create chip with thumbnail, info, and remove button', () => {
      ...
      expect(chipEl.hasClass('clian-image-chip')).toBe(true);

      const thumbEl = chipEl.querySelector('.clian-image-thumb');
      expect(thumbEl).not.toBeNull();

      const infoEl = chipEl.querySelector('.clian-image-info');
      expect(infoEl).not.toBeNull();

      const removeEl = chipEl.querySelector('.clian-image-remove');
    ...
    it('remove button should delete the image and update preview', () => {
      ...
      const removeEl = firstChip.querySelector('.clian-image-remove');
