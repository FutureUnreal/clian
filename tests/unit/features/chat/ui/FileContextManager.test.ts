...
describe('FileContextManager', () => {
  ...
  it('renders current note chip and removes on click', () => {
    ...
    const indicator = findByClass(containerEl, 'clian-file-indicator');
    expect(indicator).toBeDefined();
    expect(indicator?.style.display).toBe('flex');

    const removeEl = findByClass(containerEl, 'clian-file-chip-remove');
  ...
  it('inserts dropped file references into the input', async () => {
    ...
    inputEl.value = 'Please inspect';
    inputEl.selectionStart = inputEl.selectionEnd = inputEl.value.length;

    await manager.handleDroppedFiles([
      { path: '/vault/notes/spec.md' } as unknown as File,
      { path: 'C:/temp/report.pdf' } as unknown as File,
    ]);

    expect(inputEl.value).toBe('Please inspect @notes/spec.md @C:/temp/report.pdf');
    expect(inputEl.focus).toHaveBeenCalled();

    const chips = findAllByClass(containerEl, 'clian-file-chip');
  ...
  it('warns when a dropped item is a folder', async () => {
    ...
    await manager.handleDroppedFiles([
      { path: 'C:/temp/folder' } as unknown as File,
    ]);

    expect(inputEl.value).toBe('');
    expect(NoticeMock).toHaveBeenCalledWith(
      'Dropped folders are not supported here. Use the folder button for external contexts.',
      5000
    );
  ...
  it('shows vault-relative path in @ dropdown and inserts full path on selection', () => {
    ...
    const pathEl = findByClass(containerEl, 'clian-mention-path');
  ...
  it('wires getCachedVaultFolders through VaultFolderCache.getFolders', () => {
    ...
    const folderLabel = findByClass(containerEl, 'clian-mention-name-folder');
  ...
  it('filters context files and attaches absolute path', () => {
    ...
    const nameEls = findAllByClass(containerEl, 'clian-mention-name-context');
