...
class MockElement {
  ...
  querySelectorAll(selector: string): MockElement[] {
    ...
    const match = (el: MockElement): boolean => {
      ...
      // Handle class selectors like .clian-status-panel
...
describe('StatusPanel', () => {
  ...
  describe('mount', () => {
    it('should create panel element when mounted', () => {
      ...
      expect(containerEl.querySelector('.clian-status-panel')).not.toBeNull();
    });

    it('should create hidden todo container initially', () => {
      panel.mount(containerEl as unknown as HTMLElement);

      const todoContainer = containerEl.querySelector('.clian-status-panel-todos');
  ...
  describe('updateTodos', () => {
    ...
    it('should display correct task count', () => {
      ...
      const label = containerEl.querySelector('.clian-status-panel-label');
    ...
    it('should show current task in collapsed header', () => {
      ...
      const current = containerEl.querySelector('.clian-status-panel-current');
    ...
    it('should render all todo items in content area', () => {
      ...
      const items = containerEl.querySelectorAll('.clian-todo-item');
    ...
    it('should apply correct status classes to items', () => {
      ...
      expect(containerEl.querySelector('.clian-todo-pending')).not.toBeNull();
      expect(containerEl.querySelector('.clian-todo-in_progress')).not.toBeNull();
      expect(containerEl.querySelector('.clian-todo-completed')).not.toBeNull();
    });
  ...
  describe('toggle', () => {
    ...
    it('should expand content on header click', () => {
      const header = containerEl.querySelector('.clian-status-panel-header');
      const content = containerEl.querySelector('.clian-status-panel-content');
    ...
    it('should show list icon in header', () => {
      const icon = containerEl.querySelector('.clian-status-panel-icon');
    ...
    it('should hide current task when expanded', () => {
      ...
      expect(containerEl.querySelector('.clian-status-panel-current')).not.toBeNull();

      header!.click();

      expect(containerEl.querySelector('.clian-status-panel-current')).toBeNull();
    });
  ...
  describe('accessibility', () => {
    ...
    it('should hide status icons from screen readers', () => {
      ...
      const icon = containerEl.querySelector('.clian-todo-status-icon');
  ...
  describe('remount', () => {
    ...
    it('should re-render subagents after remount', () => {
      ...
      const subagentsEl = containerEl.querySelector('.clian-status-panel-subagents');
  ...
  describe('removeSubagent', () => {
    ...
    it('should remove a subagent by id', () => {
      ...
      const runningText = containerEl.querySelector('.clian-status-panel-running-text');
  ...
  describe('truncateDescription', () => {
    ...
    it('should truncate long subagent descriptions', () => {
      ...
      const doneText = containerEl.querySelector('.clian-status-panel-done-text');
  ...
  describe('destroy', () => {
    it('should remove panel from DOM', () => {
      ...
      expect(containerEl.querySelector('.clian-status-panel')).toBeNull();
    });
  ...
  describe('updateSubagent', () => {
    ...
    it('should show container when subagent is added', () => {
      ...
      const containerEl2 = containerEl.querySelector('.clian-status-panel-subagents');
    ...
    it('should show done row when status changes to completed', () => {
      ...
      const doneRow = containerEl.querySelector('.clian-status-panel-done-row');
    ...
    it('should show running row for running status', () => {
      ...
      const runningRow = containerEl.querySelector('.clian-status-panel-running-row');
  ...
  describe('subagent status display', () => {
    ...
    it('should show done rows for completed and running row for running', () => {
      ...
      const doneRows = containerEl.querySelectorAll('.clian-status-panel-done-row');
    ...
    it('should update display when subagent status changes', () => {
      ...
      let runningText = containerEl.querySelector('.clian-status-panel-running-text');
      expect(runningText?.textContent).toBe('1 background task');
      ...
      runningText = containerEl.querySelector('.clian-status-panel-running-text');
  ...
  describe('bash outputs', () => {
    ...
    it('should render bash section with header and entries', () => {
      ...
      const bashContainer = containerEl.querySelector('.clian-status-panel-bash');
      expect(bashContainer).not.toBeNull();
      expect(bashContainer!.style.display).toBe('block');

      const header = containerEl.querySelector('.clian-status-panel-bash-header');
      expect(header).not.toBeNull();
      const label = header!.querySelector('.clian-tool-label');
      expect(label).not.toBeNull();
      expect(label!.textContent).toBe('Command panel');

      const entries = containerEl.querySelectorAll('.clian-status-panel-bash-entry');
    ...
    it('should collapse and expand the bash section', () => {
      ...
      const content = containerEl.querySelector('.clian-status-panel-bash-content');
      ...
      header!.click();
      expect(content!.style.display).toBe('none');
      const collapsedHeader = containerEl.querySelector('.clian-status-panel-bash-header');
      expect(collapsedHeader).not.toBeNull();
      const collapsedLabel = collapsedHeader!.querySelector('.clian-tool-label');
      expect(collapsedLabel).not.toBeNull();
      expect(collapsedLabel!.textContent).toBe('echo hello');

      header!.click();
      expect(content!.style.display).toBe('block');
      const expandedHeaderAgain = containerEl.querySelector('.clian-status-panel-bash-header');
      expect(expandedHeaderAgain).not.toBeNull();
      const expandedLabelAgain = expandedHeaderAgain!.querySelector('.clian-tool-label');
    ...
    it('should collapse and expand individual bash output entries', () => {
      ...
      const entry = containerEl.querySelector('.clian-status-panel-bash-entry');
      expect(entry).not.toBeNull();

      const entryHeader = entry!.querySelector('.clian-tool-header');
      const entryContent = entry!.querySelector('.clian-tool-content');
      ...
      const entryAfterClick = containerEl.querySelector('.clian-status-panel-bash-entry');
      const contentAfterClick = entryAfterClick!.querySelector('.clian-tool-content');
      const headerAfterClick = entryAfterClick!.querySelector('.clian-tool-header');
      ...
      const entryAfterKeydown = containerEl.querySelector('.clian-status-panel-bash-entry');
      const contentAfterKeydown = entryAfterKeydown!.querySelector('.clian-tool-content');
      const headerAfterKeydown = entryAfterKeydown!.querySelector('.clian-tool-header');
    ...
    it('should clear bash outputs via action button', () => {
      ...
      const clearButton = containerEl.querySelector('.clian-status-panel-bash-action-clear');
    ...
    it('should copy latest bash output via action button', async () => {
      ...
      const copyButton = containerEl.querySelector('.clian-status-panel-bash-action-copy');
    ...
    it('should update a running bash output to completed with output', () => {
      ...
      let entry = containerEl.querySelector('.clian-status-panel-bash-entry');
      let text = entry!.querySelector('.clian-tool-result-text');
      expect(text!.textContent).toBe('Running...');

      panel.updateBashOutput('bash-1', { status: 'completed', output: 'hello', exitCode: 0 });

      entry = containerEl.querySelector('.clian-status-panel-bash-entry');
      text = entry!.querySelector('.clian-tool-result-text');
      expect(text!.textContent).toBe('hello');

      const statusEl = entry!.querySelector('.clian-tool-status');
    ...
    it('should update a running bash output to error', () => {
      ...
      const entry = containerEl.querySelector('.clian-status-panel-bash-entry');
      const text = entry!.querySelector('.clian-tool-result-text');
    ...
    it('should set aria-expanded on the bash section header', () => {
      ...
      header!.click();
      const headerAfterCollapse = containerEl.querySelector('.clian-status-panel-bash-header');
      expect(headerAfterCollapse!.getAttribute('aria-expanded')).toBe('false');

      headerAfterCollapse!.click();
      const headerAfterExpand = containerEl.querySelector('.clian-status-panel-bash-header');
