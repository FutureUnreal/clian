...
function findRoot(container: any): any {
  return container.querySelector('.clian-plan-approval-inline');
}

function findItems(root: any): any[] {
  return root.querySelectorAll('clian-ask-item');
}

describe('InlineExitPlanMode', () => {
  it('resolves with approve-new-session and includes plan content when readable', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'clian-'));
    ...
    expect(container.querySelector('.clian-plan-permissions-list')).toBeTruthy();
  ...
  it('shows a read error when plan file cannot be read', () => {
    ...
    const root = findRoot(container);
    expect(root).toBeTruthy();
    expect(container.querySelector('.clian-plan-read-error')).toBeTruthy();
  ...
  it('supports feedback flow and Escape when input is focused', () => {
    ...
    const feedbackInput = feedbackRow.querySelector('clian-ask-custom-text');
