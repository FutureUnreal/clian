...
function createMockDeps(overrides: Partial<InputControllerDeps> = {}): InputControllerDeps & { mockAgentService: ReturnType<typeof createMockAgentService> } {
  ...
  return {
    ...
    getFileContextManager: () => createMockFileContextManager() as any,
...
describe('InputController - Message Queue', () => {
  ...
  describe('Queuing messages while streaming', () => {
    ...
    it('should clear dropped file chips after queueing', async () => {
      deps.state.isStreaming = true;
      inputEl.value = '@notes/spec.md queued message';
      const fileContextManager = createMockFileContextManager();
      deps.getFileContextManager = () => fileContextManager as any;

      await controller.sendMessage();

      expect(fileContextManager.clearDroppedFiles).toHaveBeenCalled();
    });
  ...
  describe('Sending messages', () => {
    ...
    it('should clear dropped file chips after sending', async () => {
      const fileContextManager = createMockFileContextManager();
      deps.getFileContextManager = () => fileContextManager as any;
      (deps as any).mockAgentService.query = jest.fn().mockImplementation(() => createMockStream([{ type: 'done' }]));

      inputEl.value = '@notes/spec.md check this';
      ...
      const mockTitleService = {
        generateTitle: jest.fn().mockImplementation(
          async (convId: string, _user: string, _flavor: string, callback: any) => {
            ...
            await callback(convId, { success: false, error: 'Failed' });
          }
        ),
        cancel: jest.fn(),
      };
  ...
  describe('handleApprovalRequest', () => {
    ...
    it.each([
      ...
      const items = parentEl.querySelectorAll('clian-ask-item');
      const target = items.find((item: any) => {
        const label = item.querySelector('clian-ask-item-label');
    ...
    it('should render header metadata when approvalOptions provided', async () => {
      ...
      const reasonEl = parentEl.querySelector('clian-ask-approval-reason');
      expect(reasonEl?.textContent).toBe('Command is destructive');

      const pathEl = parentEl.querySelector('clian-ask-approval-blocked-path');
      expect(pathEl?.textContent).toBe('/usr/bin/rm');

      const agentEl = parentEl.querySelector('clian-ask-approval-agent');
