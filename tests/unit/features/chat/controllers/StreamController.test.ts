...
describe('StreamController - Text Content', () => {
  ...
  describe('Agent output tool use/result', () => {
    ...
    it('unlinked TOOL_AGENT_OUTPUT tool_result is appended as assistant text (fallback)', async () => {
      const msg = createTestMessage();
      deps.state.currentContentEl = createMockEl();
      ...
      await controller.handleStreamChunk(
        { type: 'tool_result', id: 'agent-out-1', content: 'Hello from TaskOutput' },
        msg
      );

      expect(msg.content).toBe('Hello from TaskOutput');
  ...
  describe('Tool header update on input re-dispatch', () => {
    it('second tool_use with same id updates existing tool input and header', async () => {
      ...
      const nameChild = toolEl.createDiv({ cls: 'clian-tool-name' });
      nameChild.setText('Read');
      const summaryChild = toolEl.createDiv({ cls: 'clian-tool-summary' });
