...
function createMockContextRow() {
  const elements: Record<string, any> = {
    '.clian-selection-indicator': { style: { display: 'none' } },
    '.clian-canvas-indicator': { style: { display: 'none' } },
    '.clian-file-indicator': null,
    '.clian-image-preview': null,
  };
...
describe('CanvasSelectionController', () => {
  ...
  it('keeps context row visible when editor selection indicator is visible', () => {
    ...
    contextRowEl.querySelector.mockImplementation((selector: string) => {
      if (selector === '.clian-selection-indicator') return editorIndicator;
      return null;
    });
