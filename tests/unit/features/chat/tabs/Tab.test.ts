...
jest.mock('@/core/agent', () => ({
  ClianService: jest.fn().mockImplementation(() => ({
...
const createMockClianService = (overrides?: {
...
describe('Tab - Service Initialization', () => {
  describe('initializeTabService', () => {
    ...
    it('should create ClianService on first initialization', async () => {
      const options = createMockOptions();
      const tab = createTab(options);
    ...
    it('should ensureReady without session ID (just spin up process)', async () => {
      ...
      const agentModule = jest.requireMock('@/core/agent') as { ClianService: jest.Mock };
      agentModule.ClianService.mockImplementationOnce(() => createMockClianService({ ensureReady: mockEnsureReady }));
    ...
    it('should sync model selector ready state with service readiness', async () => {
      ...
      const agentModule = jest.requireMock('@/core/agent') as { ClianService: jest.Mock };
      agentModule.ClianService.mockImplementationOnce(() => createMockClianService({ onReadyStateChange: mockOnReadyStateChange }));
