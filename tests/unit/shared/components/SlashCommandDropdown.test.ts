 ...
 * Each rendered item has a `.clian-slash-name` span (text: `/{name}`)
 * and an optional `.clian-slash-desc` div.
 */
function getRenderedItems(containerEl: any): { name: string; description: string }[] {
  const dropdownEl = containerEl.children.find(
    (c: any) => c.hasClass('clian-slash-dropdown')
  );
  if (!dropdownEl) return [];
  const items = dropdownEl.querySelectorAll('.clian-slash-item');
  return items.map((item: any) => {
    const nameSpan = item.children.find((c: any) => c.hasClass('clian-slash-name'));
    const descDiv = item.children.find((c: any) => c.hasClass('clian-slash-desc'));
    return {
...
// Commands that should be filtered out (not shown in Clian)
