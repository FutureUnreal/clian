...
describe('StorageService migration', () => {
  it('clears data.json after successful legacy content migration', async () => {
    ...
    expect(files.has('.clian/commands/review.md')).toBe(true);
  ...
  it('does not clear data.json when legacy content migration fails', async () => {
    ...
    const { plugin } = createMockPlugin({
      dataJson: { slashCommands: [command] },
      shouldFailWrite: (path) => path.startsWith('.clian/commands/'),
    });
  ...
  it('normalizes legacy blockedCommands during settings migration', async () => {
    ...
    const saved = JSON.parse(files.get('.clian/settings.json') || '{}') as Record<string, unknown>;
  ...
  it('does not migrate legacy activeConversationId from data.json', async () => {
    ...
    const rawSettings = files.get('.clian/settings.json');
  ...
  it('migrates data.json state fields to clian settings when empty', async () => {
    // Migration only writes when the target field is falsy.
    // Default lastClaudeModel='claude-haiku-4-5' (truthy) → won't overwrite
    ...
    // lastClaudeModel defaults to a truthy value, so migration doesn't overwrite it
    expect(saved.lastClaudeModel).toBe('claude-haiku-4-5');
    expect(saved.lastCustomModel).toBe('custom-model');
  });

  it('does not overwrite existing settings fields from data.json', async () => {
    const { plugin, files } = createMockPlugin({
      dataJson: {
      ...
      initialFiles: {
        '.clian/settings.json': JSON.stringify({
          userName: 'Test User',
  ...
  it('skips existing slash commands during migration', async () => {
    ...
    const { plugin, files } = createMockPlugin({
      dataJson: { slashCommands: [command] },
      initialFiles: {
        '.clian/commands/review.md': 'Existing content',
      },
    });
    ...
    expect(files.get('.clian/commands/review.md')).toBe('Existing content');
  });
  ...
  it('migrates lastClaudeModel from data.json when clian settings has falsy value', async () => {
    const { plugin, files } = createMockPlugin({
      dataJson: {
        lastClaudeModel: 'claude-3-sonnet',
      },
  ...
  it('preserves persistentExternalContextPaths from existing settings', async () => {
    ...
    const { plugin, files } = createMockPlugin({
      dataJson: null,
      initialFiles: {
        '.clian/settings.json': JSON.stringify(existingSettings),
      },
    });
