...
describe('SkillStorage', () => {
  it('exports SKILLS_PATH', () => {
    expect(SKILLS_PATH).toBe('.clian/skills');
  });

  describe('loadAll', () => {
    it('loads skills from subdirectories with SKILL.md', async () => {
      const adapter = createMockAdapter({
        '.clian/skills/my-skill/SKILL.md': `---
...
Do the thing`,
    ...
    it('loads multiple skills', async () => {
      const adapter = createMockAdapter({
        '.clian/skills/skill-a/SKILL.md': `---
description: Skill A
---
Prompt A`,
        '.clian/skills/skill-b/SKILL.md': `---
description: Skill B
...
Prompt B`,
    ...
    it('skips folders without SKILL.md', async () => {
      const adapter = createMockAdapter({
        '.clian/skills/has-skill/SKILL.md': `---
description: Valid
---
Prompt`,
        '.clian/skills/no-skill/README.md': 'Just a readme',
      });
      const storage = new SkillStorage(adapter);
      const skills = await storage.loadAll();
    ...
    it('skips malformed skill and continues loading valid ones', async () => {
      const adapter = createMockAdapter({
        '.clian/skills/good/SKILL.md': `---
description: Valid
---
Prompt`,
        '.clian/skills/bad/SKILL.md': 'content',
      });
...
Full prompt`,
    ...
    it('loads skills without frontmatter as content-only', async () => {
      const adapter = createMockAdapter({
        '.clian/skills/valid/SKILL.md': `---
description: Valid
---
Prompt`,
        '.clian/skills/invalid/SKILL.md': 'No frontmatter at all',
      });
      const storage = new SkillStorage(adapter);
      const skills = await storage.loadAll();
  ...
  describe('save', () => {
    it('writes skill to correct path', async () => {
      ...
      expect(adapter.ensureFolder).toHaveBeenCalledWith('.clian/skills/my-skill');
...
Prompt`,
      ...
      expect(adapter.delete).toHaveBeenCalledWith('.clian/skills/target/SKILL.md');
      expect(adapter.deleteFolder).toHaveBeenCalledWith('.clian/skills/target');
    });
  });
});
