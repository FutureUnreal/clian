...
describe('McpStorage', () => {
  describe('load', () => {
    ...
    it('loads servers with disabledTools from _clian metadata', async () => {
      const config = {
        mcpServers: {
      ...
      const adapter = createMockAdapter({
        '.clian/mcp.json': JSON.stringify(config),
      });
      const storage = new McpStorage(adapter);
      const servers = await storage.load();
    ...
    it('returns empty array when mcpServers is not an object', async () => {
      const adapter = createMockAdapter({
        '.clian/mcp.json': JSON.stringify({ mcpServers: 'invalid' }),
      });
      const storage = new McpStorage(adapter);
      const servers = await storage.load();
      expect(servers).toEqual([]);
    ...
    it('applies defaults when no metadata exists', async () => {
      const config = {
        mcpServers: {
          alpha: { command: 'alpha-cmd' },
        },
    ...
    it('loads description from _clian metadata', async () => {
      const config = {
  ...
  describe('save', () => {
    it('saves disabledTools to _clian metadata', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.clian/mcp.json']);
      expect(saved._clian.servers.alpha.disabledTools).toEqual(['tool_a', 'tool_b']);
    });
    ...
    it('omits disabledTools from metadata when empty', async () => {
      ...
      const saved = JSON.parse(adapter._store['.clian/mcp.json']);
      // No _clian since all fields are default
      expect(saved._clian).toBeUndefined();
    });

    it('preserves existing _clian extra fields when saving', async () => {
      const existing = {
        mcpServers: {
          alpha: { command: 'alpha-cmd' },
        },
      ...
      const adapter = createMockAdapter({
        '.clian/mcp.json': JSON.stringify(existing),
      });
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.clian/mcp.json']);
      expect(saved._clian.customField).toBe('should be preserved');
      expect(saved._clian.servers.alpha.disabledTools).toEqual(['tool_a']);
    });
    ...
    it('saves description to _clian metadata', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.clian/mcp.json']);
      expect(saved._clian.servers.alpha.description).toBe('A test server');
    });

    it('stores enabled=false in _clian when different from default', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.clian/mcp.json']);
      expect(saved._clian.servers.alpha.enabled).toBe(false);
    });

    it('stores contextSaving=false in _clian when different from default', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.clian/mcp.json']);
      expect(saved._clian.servers.alpha.contextSaving).toBe(false);
    });

    it('removes _clian.servers when all metadata is default', async () => {
      const existing = {
        mcpServers: { alpha: { command: 'cmd' } },
        _clian: { servers: { alpha: { enabled: false } } },
      };
      const adapter = createMockAdapter({
        '.clian/mcp.json': JSON.stringify(existing),
      });
    ...
    it('preserves non-servers _clian fields when removing servers', async () => {
      const existing = {
        mcpServers: { alpha: { command: 'cmd' } },
        _clian: {
          customField: 'keep',
      ...
      const saved = JSON.parse(adapter._store['.clian/mcp.json']);
      expect(saved._clian).toEqual({ customField: 'keep' });
    });

    it('handles corrupted existing file gracefully', async () => {
      const adapter = createMockAdapter({
        '.clian/mcp.json': 'not json',
      });
      const storage = new McpStorage(adapter);
    ...
    it('drops extra top-level fields in existing file', async () => {
      const existing = {
      ...
      const saved = JSON.parse(adapter._store['.clian/mcp.json']);
      expect(saved.someExtraField).toBeUndefined();
