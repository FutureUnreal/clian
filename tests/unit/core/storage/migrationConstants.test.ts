import {
  ...
  MIGRATABLE_CLIAN_FIELDS,
} from '@/core/storage/migrationConstants';

describe('migrationConstants', () => {
  describe('CLIAN_ONLY_FIELDS', () => {
    it('contains all expected user preference fields', () => {
      expect(CLIAN_ONLY_FIELDS.has('userName')).toBe(true);
    });

    it('contains all expected security settings', () => {
      expect(CLIAN_ONLY_FIELDS.has('enableBlocklist')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('blockedCommands')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('permissionMode')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('lastNonPlanPermissionMode')).toBe(true);
    });

    it('contains all expected model & thinking fields', () => {
      expect(CLIAN_ONLY_FIELDS.has('model')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('thinkingBudget')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('enableAutoTitleGeneration')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('titleGenerationModel')).toBe(true);
    });

    it('contains all expected content settings', () => {
      expect(CLIAN_ONLY_FIELDS.has('excludedTags')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('mediaFolder')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('systemPrompt')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('allowedExportPaths')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('persistentExternalContextPaths')).toBe(true);
    });

    it('contains all expected environment fields', () => {
      expect(CLIAN_ONLY_FIELDS.has('environmentVariables')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('envSnippets')).toBe(true);
    });

    it('contains all expected UI settings', () => {
      expect(CLIAN_ONLY_FIELDS.has('keyboardNavigation')).toBe(true);
    });

    it('contains all expected CLI path fields', () => {
      expect(CLIAN_ONLY_FIELDS.has('claudeCliPath')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('claudeCliPaths')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('loadUserClaudeSettings')).toBe(true);
    });

    it('contains deprecated fields', () => {
      expect(CLIAN_ONLY_FIELDS.has('allowedContextPaths')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('showToolUse')).toBe(true);
      expect(CLIAN_ONLY_FIELDS.has('toolCallExpandedByDefault')).toBe(true);
    });
  });

  describe('MIGRATABLE_CLIAN_FIELDS', () => {
    it('contains user preferences', () => {
      expect(MIGRATABLE_CLIAN_FIELDS.has('userName')).toBe(true);
    });

    it('contains security settings', () => {
      expect(MIGRATABLE_CLIAN_FIELDS.has('enableBlocklist')).toBe(true);
      expect(MIGRATABLE_CLIAN_FIELDS.has('blockedCommands')).toBe(true);
      expect(MIGRATABLE_CLIAN_FIELDS.has('permissionMode')).toBe(true);
    });

    it('contains model settings', () => {
      expect(MIGRATABLE_CLIAN_FIELDS.has('model')).toBe(true);
      expect(MIGRATABLE_CLIAN_FIELDS.has('thinkingBudget')).toBe(true);
    });

    it('contains environment fields including legacy env', () => {
      expect(MIGRATABLE_CLIAN_FIELDS.has('environmentVariables')).toBe(true);
      expect(MIGRATABLE_CLIAN_FIELDS.has('env')).toBe(true);
    });

    it('excludes deprecated fields', () => {
      expect(MIGRATABLE_CLIAN_FIELDS.has('allowedContextPaths')).toBe(false);
      expect(MIGRATABLE_CLIAN_FIELDS.has('showToolUse')).toBe(false);
    });
  });
...
NEW_FEATURE=true`;
  ...
  describe('integration scenarios', () => {
    ...
    it('handles real-world Claude Code environment migration', () => {
      ...
      const extraEnv = 'CLAUDE_CLI_PATH=/usr/local/bin/claude\nENABLE_FEATURE=true';

      const ccEnvStr = convertEnvObjectToString(ccEnv);
      const merged = mergeEnvironmentVariables(ccEnvStr, extraEnv);
