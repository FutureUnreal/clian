...
describe('StorageService convenience methods', () => {
  ...
  const clianSettingsJson = JSON.stringify({
    userName: 'Test',
    model: 'haiku',
    permissionMode: 'yolo',
  });
  ...
  describe('updateClianSettings', () => {
    it('updates partial clian settings', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.clian/settings.json': clianSettingsJson,
        },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();

      await storage.updateClianSettings({ userName: 'NewUser' });
  ...
  describe('saveClianSettings', () => {
    it('saves full clian settings', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.clian/settings.json': clianSettingsJson,
        },
      ...
      const existing = await storage.loadClianSettings();
      existing.userName = 'FullSave';
      await storage.saveClianSettings(existing);
  ...
  describe('loadClianSettings', () => {
    it('loads clian settings', async () => {
      const { plugin } = createMockPlugin({
        initialFiles: {
          '.clian/settings.json': clianSettingsJson,
        },
      ...
      const settings = await storage.loadClianSettings();
  ...
  describe('getLegacyActiveConversationId', () => {
    it('returns id from clian settings', async () => {
      const settings = JSON.stringify({
        userName: 'Test',
    ...
    it('falls back to data.json when not in clian settings', async () => {
      const { plugin } = createMockPlugin({
        dataJson: { activeConversationId: 'conv-from-data' },
        initialFiles: {
          '.clian/settings.json': JSON.stringify({ userName: 'Test' }),
        },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();
