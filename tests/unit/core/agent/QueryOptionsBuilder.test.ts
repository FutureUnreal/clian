...
import { type ClianSettings,DEFAULT_SETTINGS } from '@/core/types';
...
function createMockSettings(overrides: Partial<ClianSettings> = {}): ClianSettings {
  return {
    ...DEFAULT_SETTINGS,
...
describe('QueryOptionsBuilder', () => {
  ...
  describe('buildPersistentQueryConfig', () => {
    it('builds config with default settings', () => {
      ...
      expect(config.model).toBe(DEFAULT_SETTINGS.model);
