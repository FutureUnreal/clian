...
import { ClianService } from '@/core/agent/ClianService';
...
describe('ClianService', () => {
  let mockPlugin: Partial<ClianPlugin>;
  let mockMcpManager: MockMcpServerManager;
  let service: ClianService;
  ...
  beforeEach(() => {
    ...
    service = new ClianService(mockPlugin as ClianPlugin, mockMcpManager);
  });
  ...
  describe('isStreamTextEvent', () => {
    ...
    it('should return true for content_block_start with text type', () => {
      expect((service as any).isStreamTextEvent({
        type: 'stream_event',
        event: { type: 'content_block_start', content_block: { type: 'text', text: 'hi' } },
      })).toBe(true);
    });
  ...
  describe('routeMessage', () => {
    ...
    it('should dedupe assistant text against streamed text per content block', async () => {
      const streamMsg = {
        type: 'stream_event',
      ...
      await (service as any).routeMessage(streamMsg);
      await (service as any).routeMessage(assistantMsg);

      const textChunks = onChunk.mock.calls
        .filter(([chunk]: any) => chunk.type === 'text')
        .map(([chunk]: any) => chunk.content);
      expect(textChunks).toEqual(['hello']);
    });

    it('should emit only the missing delta from terminal assistant text', async () => {
      const streamStart = {
        type: 'stream_event',
        event: { type: 'content_block_start', index: 0, content_block: { type: 'text', text: 'hel' } },
      };
      const streamDelta = {
        type: 'stream_event',
      ...
      await (service as any).routeMessage(streamStart);
      await (service as any).routeMessage(streamDelta);
      await (service as any).routeMessage(assistantMsg);

      const textChunks = onChunk.mock.calls
        .filter(([chunk]: any) => chunk.type === 'text')
        .map(([chunk]: any) => chunk.content);
      expect(textChunks).toEqual(['hel', 'lo', ' world']);
    });
